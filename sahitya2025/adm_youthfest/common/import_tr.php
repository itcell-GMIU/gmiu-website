...
if (isset($_POST["import"])) {
    ...
    if ($_FILES["file"]["size"] > 0) {
        ...
        while (($getData = fgetcsv($file, 10000, ",")) !== FALSE) {
            ...
            $fees = $getData[1];
            $payment_status = $getData[2];
            $contact = $getData[8];

            $stmt = $con->prepare("UPDATE tbl_registers SET fee_amount = ?, payment_status = ? WHERE mobile = ?");
            $stmt->bind_param("iss",$fees,$payment_status,$contact);
            $result = $stmt->execute();
        }
