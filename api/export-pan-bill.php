...
if (!empty($from_date) && !empty($to_date)) {
    ...
    $filename = "Practical_Exam_Bill_PAN_" . $from_date . "_to_" . $to_date . ".xls";
} else {
    $date_string = "All Dates";
    $filename = "Practical_Exam_Bill_PAN_" . date('Y-m-d_H-i-s') . ".xls";
}
...
$sql = "SELECT id, full_name, form_date, subject_code, subject_name, duty_type, phone_no, pan_card, 
...
if ($result && $result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        if ($group == 'EXTERNAL EXAMINER') $displayCategory = 'EXTERNAL';
        if ($group == 'INTERNAL EXAMINER') $displayCategory = 'INTERNAL';
        ...
        $allRows[] = [
            ...
            'category_short' => $displayCategory
        ];
...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
 ...
 <Worksheet ss:Name="Practical Exam Bill PAN">
  <Table>
   <!-- Columns widths -->
   <Column ss:Width="40"/>
   <Column ss:Width="130"/>
   ...
   <!-- Title Rows -->
   <Row ss:Height="25">
    <Cell ss:MergeAcross="13" ss:StyleID="sTitle1"><Data ss:Type="String">GYANAMNAJARI INNOVATIVE UNIVERSITY</Data></Cell>
   </Row>
   <Row ss:Height="20">
    <Cell ss:MergeAcross="13" ss:StyleID="sTitle2"><Data ss:Type="String">PRACTICAL EXAM BILL SUMMER <?= date('Y') ?></Data></Cell>
   </Row>
   <Row ss:Height="18">
    <Cell ss:MergeAcross="13" ss:StyleID="sTitle3"><Data ss:Type="String">Examination Date:- <?= $date_string ?></Data></Cell>
   </Row>
   ...
   <Row ss:Height="20">
    ...
    <Cell ss:MergeAcross="5" ss:StyleID="sHeaderGray"><Data ss:Type="String">Amounts</Data></Cell>
    <Cell ss:MergeDown="1" ss:StyleID="sHeader"><Data ss:Type="String">Contact No.</Data></Cell>
    <Cell ss:MergeDown="1" ss:StyleID="sHeader"><Data ss:Type="String">PAN Number</Data></Cell>
   </Row>
   <Row ss:Height="30">
    ...
    <Cell ss:StyleID="sHeader"><Data ss:Type="String">FINAL TOTAL</Data></Cell>
   </Row>
   ...
   foreach ($allRows as $row) {
       if (empty($row['pan_card'])) {
           $emptyPanRows[] = [$row]; // keep them separate
       } else {
           $panGroups[$row['pan_card']][] = $row;
       }
   }
   
   foreach ($panGroups as $groupRows) {
       $processedGroups[] = $groupRows;
       }
   foreach ($emptyPanRows as $groupRows) {
       $processedGroups[] = $groupRows;
   }

   foreach ($processedGroups as $groupRows) {
       $span = count($groupRows);
       
       $dutyTypes = [];
       $panTotal = 0;
       foreach ($groupRows as $r) {
           if (!in_array($r['category_short'], $dutyTypes)) {
               $dutyTypes[] = $r['category_short'];
           }
           $panTotal += floatval($r['total']);
       }
       $mergedDutyStr = implode(' / ', $dutyTypes);
       
       for ($k = 0; $k < $span; $k++) {
           $row = $groupRows[$k];
           echo "<Row ss:AutoFitHeight=\"1\" ss:Height=\"20\">";
                   
           // SR No
           echo "<Cell ss:Index=\"1\" ss:StyleID=\"sDataCenter\"><Data ss:Type=\"Number\">" . $srNo++ . "</Data></Cell>";
                   
           $mergeAttr = ($span > 1 && $k == 0) ? " ss:MergeDown=\"" . ($span - 1) . "\"" : "";
                   
           // TYPE OF EXAMINER
           if ($k == 0 || $span == 1) {
               echo "<Cell ss:Index=\"2\"" . $mergeAttr . " ss:StyleID=\"sDataLeft\"><Data ss:Type=\"String\">" . htmlspecialchars($mergedDutyStr) . "</Data></Cell>";
           }
                   
           // DATE, SUB CODE, SUB NAME
           echo "<Cell ss:Index=\"3\" ss:StyleID=\"sDataCenter\"><Data ss:Type=\"String\">" . htmlspecialchars($row['date']) . "</Data></Cell>";
           echo "<Cell ss:Index=\"4\" ss:StyleID=\"sDataCenter\"><Data ss:Type=\"String\">" . htmlspecialchars($row['subject_code']) . "</Data></Cell>";
           echo "<Cell ss:Index=\"5\" ss:StyleID=\"sDataCenter\"><Data ss:Type=\"String\">" . htmlspecialchars($row['subject_name']) . "</Data></Cell>";
                   
           // NAME OF THE PERSON
           if ($k == 0 || $span == 1) {
               echo "<Cell ss:Index=\"6\"" . $mergeAttr . " ss:StyleID=\"sDataCenter\"><Data ss:Type=\"String\">" . htmlspecialchars($groupRows[0]['name']) . "</Data></Cell>";
           }
                   
           // TA, DA, HON, ACC, TOTAL
           echo "<Cell ss:Index=\"7\" ss:StyleID=\"sDataCenter\"><Data ss:Type=\"String\">" . (empty($row['ta']) ? '-' : $row['ta']) . "</Data></Cell>";
           echo "<Cell ss:Index=\"8\" ss:StyleID=\"sDataCenter\"><Data ss:Type=\"String\">" . (empty($row['da']) ? '-' : $row['da']) . "</Data></Cell>";
           echo "<Cell ss:Index=\"9\" ss:StyleID=\"sDataCenter\"><Data ss:Type=\"String\">" . (empty($row['honorarium']) ? '-' : $row['honorarium']) . "</Data></Cell>";
           echo "<Cell ss:Index=\"10\" ss:StyleID=\"sDataCenter\"><Data ss:Type=\"String\">" . (empty($row['accommodation']) ? '-' : $row['accommodation']) . "</Data></Cell>";
           echo "<Cell ss:Index=\"11\" ss:StyleID=\"sDataBold\"><Data ss:Type=\"String\">" . (empty($row['total']) ? '-' : $row['total']) . "</Data></Cell>";
                   
           // FINAL TOTAL
           if ($k == 0 || $span == 1) {
               echo "<Cell ss:Index=\"12\"" . $mergeAttr . " ss:StyleID=\"sDataBold\"><Data ss:Type=\"String\">" . ($panTotal == 0 ? '-' : $panTotal) . "</Data></Cell>";
           }
                   
           // Contact No.
           if ($k == 0 || $span == 1) {
               echo "<Cell ss:Index=\"13\"" . $mergeAttr . " ss:StyleID=\"sDataCenter\"><Data ss:Type=\"String\">" . htmlspecialchars($groupRows[0]['contact']) . "</Data></Cell>";
           }
                   
           // PAN Number
           if ($k == 0 || $span == 1) {
               echo "<Cell ss:Index=\"14\"" . $mergeAttr . " ss:StyleID=\"sDataCenter\"><Data ss:Type=\"String\">" . htmlspecialchars($groupRows[0]['pan_card']) . "</Data></Cell>";
           }
   ...
   <!-- Summary Chart at the bottom exactly like original logic -->
   <Row>
