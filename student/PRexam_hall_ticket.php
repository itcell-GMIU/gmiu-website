        ...
        ?>
            <div class="receipt-container">
                    ...
                    <div class="col-12 p-0 text-center">
                        <table class="table w-100 border-collapse" style="border:1px solid black;">
                            <tbody>
                                <tr>
                                    ...
                                    while ($row2 = $result2->fetch_assoc()) {
                                        ...
                                        foreach ($subjectArray as $subject_code) {
                                            $examtt = $con->prepare("SELECT `subject_code` FROM `tbl_exam_timetable` WHERE `subject_code`= ? and `sub_type`= 'practical' ");
                                    ...
                                    ?>
                                                <td style="width: 126px;border:1px solid black;"><?= $subject_c ?></td>
                                        <?php }
                                        } ?>
                                </tr>
                                <tr>
                                    ...
                                    <?php
                                        ...
                                        $subjectArray = explode(', ',  $subject_codes);
                                        foreach ($subjectArray as $subject_code) {
                                            $examtt = $con->prepare("SELECT `date` FROM `tbl_exam_timetable` WHERE `subject_code`= ? and `sub_type`= 'practical' ");
                                            $examtt->bind_param("s", $subject_code);
                                            $examtt->execute();
                                            $examttres = $examtt->get_result();
                                            while ($rowtt = $examttres->fetch_assoc()) {
                                            ...
                                            if ($examttres->num_rows > 0) {
                                    ?>
                                            <td style="width: 126px;border:1px solid black;"><?= $subject_date ?></td>
                                    <?php }
                                        } ?>
                                </tr>
                                <tr>
                                    ...
                                    <?php
                                        ...
                                        $subjectArray = explode(', ',  $subject_codes);
                                        foreach ($subjectArray as $subject_code) {
                                            $examtt = $con->prepare("SELECT `start_time`, `end_time` FROM `tbl_exam_timetable` WHERE `subject_code`= ? and `sub_type`= 'practical' ");
                                            $examtt->bind_param("s", $subject_code);
                                            $examtt->execute();
                                            $examttres = $examtt->get_result();
                                            while ($rowtt = $examttres->fetch_assoc()) {
                                                ...
                                                $end_timeN = date("h:i A", strtotime($end_time));
                                            }
                                            if ($examttres->num_rows > 0) {
                                                if ($end_time == NULL || empty($end_time) || $end_time == "00:00:00") {
                                                    ?>
                                                    <td style="width: 126px;border:1px solid black;"><?= $start_time ?></td>
                                                    <?php
                                                }else{
                                    ?>
                                            <td style="width: 126px;border:1px solid black;"><?= $start_time . ' To ' . $end_timeN ?></td>
                                <?php }
                                ...
                                <tr>
                                    <td style="border:1px solid black;">Signature of Examiner</td>
                                    <td style="width: 126px;border:1px solid black;"></td>
                                    <td style="width: 126px;border:1px solid black;"></td>
                                    <td style="width: 126px;border:1px solid black;"></td>
                                    <td style="width: 126px;border:1px solid black;"></td>
                                    <td style="width: 126px;border:1px solid black;"></td>
                ...
                <div class="text-left mt-5">
                    ...
                    <ul>
                        <li>Students must be 30 minutes before of exam start time at their exam location.</li>
                        <li>Bring Your Practical Journal. [B.Sc./M.Sc.]</li>
                        <li>*# Question paper will be 50 marks MCQ. [BA/B.COM/BBA/B.Sc./BCA]</li>
                    </ul>
