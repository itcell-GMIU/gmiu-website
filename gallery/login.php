...
include("include/connect.php"); // Path adjusted for the new file structure
session_start();
...
if (isset($_POST['btn-login'])) { // This now works because name="btn-login" is added to the button
    ...
    $cmd = $con->prepare("SELECT id as role_id, `id` as staff_id, `email` as staff_email from gallery_admin WHERE email= ? AND password= ? AND id= ? AND is_active=1 AND is_delete=0;");
			...
			<form method="POST" action="">
				<div class="input-group custom input-group-lg mb-3">
					<select class="form-control" name="role" required>
						...
						<?php
						$query = "SELECT id,name FROM gallery_admin WHERE is_active = 1 and is_delete=0";
