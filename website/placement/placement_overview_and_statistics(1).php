                            ...
                            <div class="curriculum-text-box" style="margin-top: 10px; margin-bottom: 50px;">
                                <div class="curriculum-section">
                                    ...
                                    $query = "SELECT program.id AS program_id,program.name AS program_name,program.intake AS program_intake,program.duration AS program_duration,
                                            ...
                                            p.program_id = program.id WHERE program.faculty_id = faculty.id AND p.is_active = 1 AND p.is_delete = 0)) AND program.level_id IN(SELECT LEVEL.id AS level_id
                                            FROM tbl_level AS LEVEL WHERE EXISTS(SELECT 1 FROM tbl_program program INNER JOIN tbl_placement p ON p.program_id = program.id WHERE program.level_id = LEVEL.id 
                                            ...
                                            tbl_program program INNER JOIN tbl_placement p ON p.program_id = program.id WHERE program.faculty_id = faculty.id AND p.is_active = 1 AND p.is_delete = 0
                                            ...
                                            AND program.is_delete = 0 AND EXISTS(SELECT 1 FROM tbl_placement p WHERE p.is_active = 1 AND p.is_delete = 0 AND p.program_id = program.id AND 
                                                ...
                                                <div style="width:100%;
                                                    ...
                                                    <div style="padding:12px 24px;
                                                        ...
                                                        <h3 class="course-name">
                                                            <?php echo htmlspecialchars($row['program_name'], ENT_QUOTES); ?>
                                                            (<?php echo htmlspecialchars($row['level_name'], ENT_QUOTES); ?>)
                                                        </h3>
                                                    </div>
