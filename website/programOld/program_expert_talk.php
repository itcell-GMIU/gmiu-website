        ...
        <div class="container">
            <section class="placed-students">
                <div class="buttons-container-flex" style="padding: 10px;">
                    ...
                    $cmd = "SELECT YEAR(date) FROM tbl_expert_talk WHERE is_active = 1 AND is_delete = 0 and program_id = $program_id and faculty_id = $faculty_id GROUP BY YEAR(date) ORDER BY YEAR(date)DESC ";
                    ...
                    <div class="single-curses-contert">
                        ...
                        <section class="events-list-03">
                            <div class="container">
                                <?php
                                //code for getting year buttons by grouping year in placement table
                                $cmd6 = "SELECT YEAR(date) FROM tbl_expert_talk WHERE is_active = 1 AND is_delete = 0 and program_id = $program_id and faculty_id = $faculty_id GROUP BY YEAR(date) ORDER BY YEAR(date)";
                                ...
                                ?>
                                    <div class="row event-body-content semester-content" id="sem-<?php echo $sem_c; ?>" style="display: none;">
                                        <?php
                                        $cmd = $con->prepare("SELECT expert_talk.id as expert_talk_id,expert_talk.title as expert_talk_title, expert_talk.description as expert_talk_description FROM tbl_expert_talk as expert_talk  WHERE program_id=? AND is_active=1 AND is_delete=0 AND YEAR(date) = $sem_c");
                                        $cmd->bind_param("i", $program_id);
                                        $cmd->execute();
                                        $result = $cmd->get_result();
                                        if ($result->num_rows > 0) {
