                    ...
                    <div class="single-curses-contert">
                        ...
                        <section class="placement-overview">
                            <div class="buttons-container-flex">
                                ...
                                $cmd = "SELECT year FROM tbl_placement WHERE is_active = 1 AND is_delete = 0 and FIND_IN_SET(?, program_id) and faculty_id = ? GROUP BY year ORDER BY year DESC";
                                $stmt = $con->prepare($cmd);
                                $stmt->bind_param("ss", $program_id, $faculty_id);
                                $stmt->execute();
                        ...
                        <section class="placed-students">
                            ...
                            // $stmt6->execute();
                            // $result6 = $stmt6->get_result();
                            $cmd6 = "SELECT year FROM tbl_placement WHERE is_active = 1 AND is_delete = 0 and FIND_IN_SET(?, program_id) and faculty_id = ? GROUP BY year ORDER BY year DESC";
                            $stmt6 = $con->prepare($cmd6);
                            $stmt6->bind_param("ss", $program_id, $faculty_id);
                            $stmt6->execute();
                            $result6 = $stmt6->get_result();
                            ...
                            ?>
                                <div class="placement-cards-container semester-content" id="sem-<?php echo $sem_c; ?>" style="display: none;">
                                    <div class="row container-plc">
                                        <?php
                                        $cmd9 = $con->prepare("SELECT `student_name`, `student_image`, `company_logo`, `package` FROM tbl_placement WHERE FIND_IN_SET(?, program_id) AND is_active=1 AND is_delete=0 AND year = $sem_c");
                                        $cmd9->bind_param("s", $program_id);
                                        $cmd9->execute();
                                        ...
                                        ?> <!-- //code for getting year wise student -->
                                                <div class="col-sm-3 card-plc">
                                                    <img class="plc-std" src="<?php echo $upload_website_admin_url; ?>placement/student_image/<?php echo $student_image; ?>" alt="Avatar" style="height:150px; object-fit: contain;">
                                                    <div class="container-plc">
                                                        <h5 style="margin: 0; padding:0;"><?php echo $student_name; ?></h5>
                                                    </div>
                                                    <img class="plc-logo" src="<?php echo $upload_website_admin_url; ?>placement/company_logo/<?php echo $company_logo; ?>" alt="Avatar" style="width:100%; object-fit: contain; ">
                                                </div>
                                        <?php }
                                        }
                                        ?>
