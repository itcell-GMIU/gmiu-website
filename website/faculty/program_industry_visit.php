...
if (isset($_GET['program_slug']) && !empty($_GET['program_slug'])) {
    ...
    program.id as program_id, program.description as program_description, program.industry_meta_description from tbl_program as program LEFT JOIN tbl_level as level ON program.level_id = level.id
    ...
    if ($result->num_rows != 0) {
        ...
        $meta_description = $row['industry_meta_description'];
...
<div id="preloader">
    ...
    <section class="hero">
        ...
        <div class="container">
            <div class="cont">
                <div class="top">
                    <h1><?php echo $program_name; ?> Industry Visit</h1>
                </div>
                <p style="margin-top:5px;">
                    ...
                    <span class="b-active"><a
                        ...
                        <i class='fa fa-angle-right'></i></span>
                    <span class="b-active"><a href="">Industry Visit</a></span>
                </p>
                <hr>
            </div>
        ...
        <div class="container">
            <section class="placed-students">
                <div class="buttons-container-flex" style="padding: 10px;">
                    ...
                    $cmd = "SELECT YEAR(date) FROM tbl_industry_visit WHERE is_active = 1 AND is_delete = 0 and FIND_IN_SET('$program_id', program_id) and faculty_id = $faculty_id GROUP BY YEAR(date) ORDER BY YEAR(date)DESC ";
                    ...
                    <div class="single-curses-contert">
                        ...
                        <section class="events-list-03">
                            <div class="container">
                                <?php
                                //code for getting year buttons by grouping year in placement table
                                $cmd6 = "SELECT YEAR(date) FROM tbl_industry_visit WHERE is_active = 1 AND is_delete = 0 and FIND_IN_SET('$program_id', program_id) and faculty_id = $faculty_id GROUP BY YEAR(date) ORDER BY YEAR(date) DESC";
                                ...
                                ?>
                                    <div class="row event-body-content semester-content" id="sem-<?php echo $sem_c; ?>"
                                        style="display: none;">
                                        <?php
                                        $cmd = $con->prepare("SELECT industry_visit.id as industry_visit_id,industry_visit.visit_name as industry_visit_title,industry_visit.report as industry_visit_report, industry_visit.visit_description as industry_visit_description FROM tbl_industry_visit as industry_visit  WHERE FIND_IN_SET('$program_id', program_id) AND is_active=1 AND is_delete=0 AND YEAR(date) = $sem_c");
                                        //  $cmd->bind_param("i", $program_id);
                                        $cmd->execute();
                                        $result = $cmd->get_result();
                                        if ($result->num_rows > 0) {
                                            while ($row = $result->fetch_assoc()) {
                                                $industry_visit_title = $row['industry_visit_title'];
                                                $industry_visit_description = $row['industry_visit_description'];
                                                $industry_visit_id = $row['industry_visit_id'];
                                                $industry_visit_report = $row['industry_visit_report'];
                                        ?>
                                                <!-- INSIDE THE INNER LOOP WHERE YOU DISPLAY EACH INDUSTRY VISIT CARD, REPLACE THIS SECTION -->
                                                ...
                                                <div class="col-sm-8 events-full-box">
                                                    <div class="events-single-box">
                                                        ...
                                                        <div class="row">
                                                            ...
                                                            <div class="col-12">
                                                                <div
                                                                    class="swiper swipermain mySwipers<?php echo $industry_visit_id; ?>">
                                                                    <div class="swiper-wrapper">
                                                                        <?php
                                                                        $type = "industry_visit";
                                                                        $cmd1 = $con->prepare("SELECT sp.file_name FROM tbl_site_photos AS sp WHERE type_id=? AND type=? AND is_active=1 AND is_delete=0");
                                                                        $cmd1->bind_param("is", $industry_visit_id, $type);
                                                                        $cmd1->execute();
                                                                        $result1 = $cmd1->get_result();
                                                                        while ($row1 = $result1->fetch_assoc()) {
                                                                            echo "<div class='swiper-slide'>
                                                                                    <img src='" . $upload_website_admin_url . "industry_visit/image/" . htmlspecialchars($row1['file_name']) . "' 
                                                            ...
                                                            <!-- THEN DESCRIPTION SECTION -->
                                                            <div class="col-12 mt-3">
                                                                <div class="event-info">
                                                                    <p class="events-time">
                                                                        <?php echo htmlspecialchars_decode($industry_visit_description); ?>
                                                                    </p>
                                                                    <a target="_blank"
                                                                        href="<?php echo $upload_website_admin_url; ?>industry_visit/report/<?php echo $industry_visit_report ?>">
                                                                        <button id="dwn-btn">
    ...
    <script>
        <?php
        // Get all industry_visit IDs for Swiper initialization
        $cmd = $con->prepare("SELECT id FROM tbl_industry_visit WHERE FIND_IN_SET(?, program_id) AND is_active=1 AND is_delete=0 AND faculty_id=?");
        $cmd->bind_param("ii", $program_id, $faculty_id);
        $cmd->execute();
        $result = $cmd->get_result();
        while ($row = $result->fetch_assoc()) {
            $industry_visit_id = $row['id'];
            echo '
    var swiper' . $industry_visit_id . ' = new Swiper(".mySwipers' . $industry_visit_id . '", {
        loop: true,
        spaceBetween: 10,
        pagination: {
