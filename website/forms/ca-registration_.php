...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $course_type = $_POST['course_type']; // Capture the hidden course type field
    ...
    // --- Insert into the new tbl_courses_form table ---
    $sql = "INSERT INTO tbl_courses_form (course_type, full_name, mobile_number, standard, district_id, school_name, email_id, start_date, end_date, preferred_timing_slot) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    $stmt->bind_param("ssssisssss", $course_type, $full_name, $mobile_number, $standard, $district, $school_name, $email_id, $start_date, $end_date, $timing_slot);

    if ($stmt->execute()) {
        $safe_full_name = strtoupper($full_name);
        echo <<<HTML
    <head>
...
<head>
    ...
    <title>CA Foundation Crash Course</title>
...
<body class="flex justify-center items-center min-h-screen p-4 my-10 sm:my-0">
    ...
    <div
        ...
        <h2 class="text-2xl sm:text-3xl font-bold text-gray-800 mb-2">CA Foundation Crash Course</h2>
        <p class="text-gray-600 mb-8">Fill in your details below to register.</p>

        <form method="POST" action="">
            <!-- *** ADD THIS HIDDEN FIELD *** -->
            <input type="hidden" name="course_type" value="CA">
            ...
            <div class="space-y-4 text-left">
                ...
                <div class="flex flex-col sm:flex-row gap-4">
                    <div class="relative flex-1">
                        ...
                        <input type="date" id="start_date" name="start_date" min="2025-09-01" max="2026-01-14" required
    ...
    <script>
        ...
        // *** NEW JAVASCRIPT FOR DYNAMIC SCHOOL FIELD ***
        const districtSelect = document.getElementById('district');
        const standardSelect = document.getElementById('standard');
        const schoolContainer = document.getElementById('school-container');

        // HTML for the default text input field for schools
        ...
        // Function to update the school field
        function updateSchoolField() {
            const districtId = districtSelect.value;
            const standard = standardSelect.value;
            const specialDistricts = ['2', '7', '8'];

            // Check if the selected district is one of the special ones and if a standard is selected
            if (specialDistricts.includes(districtId) && standard) {
                ...
                fetch(`get_schools.php?district_id=${districtId}&standard=${standard}`)
                    .then(response => response.json())
                    .then(schools => {
                        if (schools.length > 0) {
                            console.log(schools.length);
                            // If schools are found, build a select dropdown
                            let selectHTML = `
                                <div class="relative">
                                    <span class="absolute inset-y-0 left-0 flex items-center pl-4 text-gray-500"><i class="bi bi-building-fill"></i></span>
                            ...
                            schools.forEach(school => {
                                // Sanitize school name for the value attribute
                            ...
                            // If no schools are found for that district/standard, revert to a text input
                            schoolContainer.innerHTML = schoolInputHTML;
                        }
                    })
                    .catch(error => {
                        console.error('Error fetching schools:', error);
                        // If there's an error, revert to the text input
                        schoolContainer.innerHTML = schoolInputHTML;
                    });
            } else {
                // If the district is not special, show the default text input
                schoolContainer.innerHTML = schoolInputHTML;
            }
        }
