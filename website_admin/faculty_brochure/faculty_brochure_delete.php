...
if (isset($_GET['id']) && !empty($_GET['id'])) {
    ...
    $stmt = $con->prepare("SELECT document FROM `tbl_faculty_brochure` WHERE id = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        $path2 = '../uploads/faculty_brochure/document/';

        if (delete_file($document, $path2)) {
            $stmt = $con->prepare("UPDATE `tbl_faculty_brochure` SET is_delete = 1, is_active = 0 WHERE id = ?");
            $stmt->bind_param("i", $brochure_id);
            $result = $stmt->execute();
