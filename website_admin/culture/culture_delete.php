...
if (isset($_GET['sp_id']) && !empty($_GET['sp_id'])) {
    $id = mysqli_real_escape_string($con, $_GET['sp_id']);
    $id = only_digits($id);
    if ($id == false) {
...
if ($result) {
    ...
    $cmd = $con->prepare("SELECT photos.file_name FROM `tbl_site_photos` AS photos INNER JOIN `tbl_campus` AS culture 
    ON photos.type_id = culture.id WHERE photos.type = ? AND photos.type_id = ?");
    ...
    //   $report_name = $row['report'];
      ...
      if (delete_file($file_name, $path)) {
        ...
        if ($file_delete) {
          $_SESSION['status'] = "culture Activity is deleted successfully";
          $_SESSION['status_code'] = "success";
          
          // Delete the report file if it exists
          if (!empty($report_name)) {
            $report_path = '../uploads/culture/';
            if (delete_file($report_name, $report_path)) {
             
            }
          }
        } else {
          $_SESSION['status'] = "culture Activity deletion failed";
          $_SESSION['status_code'] = "error";
        }
      } else {
        $_SESSION['status'] = "culture Activity deletion failed";
    ...
    // Redirect to culture Activity view page after processing
    echo "<script>setTimeout(function(){window.location='culture_view.php'}, 1000);</script>";
