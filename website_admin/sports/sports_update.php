...
if (isset($_POST['submit'])) {
    ...
    $sp_id = mysqli_real_escape_string($con, $_POST['sp_id']);
    $date = $_POST['date'];
    $description = $_POST['description'];
   
    // Validate Data
    $sp_id = validate_data($sp_id);
    
    $stmt = $con->prepare("UPDATE `tbl_campus` SET  `date` = ?, `description`=? WHERE id = ?");
    $stmt->bind_param("ssi", $date, $description, $sp_id);
    $result = $stmt->execute();
    ...
    if ($_FILES['image_upload']['error'][0] == 0) {
        ...
        if ($result) {
            ...
            if ($uploaded_images['status'] == 200) {
                ...
                $_SESSION['status'] = "Sports updated successfully";
