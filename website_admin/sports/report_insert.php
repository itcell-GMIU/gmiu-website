...
if (isset($_POST['submit'])) {
    ...
    if (isset($_POST['report_type']) && !empty($_POST['report_type'])) {
        ...
        // Handle the error if report_type is not selected
        $_SESSION['status'] = "Please select a valid Activity Type";
        $_SESSION['status_code'] = "error";
        echo "<script>setTimeout(function(){window.location='ssports_insert.php'},1000)</script>";
        exit();
    }
    ...
    $stmt = $con->prepare("INSERT INTO `tbl_ssports`(report_type,report_title, date, description) VALUES (?, ?, ?, ?)");
    $stmt->bind_param("ssss", $report_type, $title, $date, $description);
    $result = $stmt->execute();
    $id = $con->insert_id;
    ...
    if ($result && $_FILES['report_upload']['error'] == 0 && !empty($_FILES['image_upload']['name'][0])) {
        ...
        if ($file_upload_status['status'] == 200) {
            ...
            $stmt = $con->prepare("UPDATE `tbl_ssports` SET `report` = ? WHERE `tbl_ssports`.`id` = ?");
            $stmt->bind_param("si", $report_name, $id);
            $stmt->execute();
        } else {
            $stmt = $con->prepare("DELETE FROM `tbl_ssports` WHERE id = ?");
        ...
        $_SESSION['status'] = "sports_report Inserted Successfully";
        $_SESSION['status_code'] = "success";
        echo "<script>setTimeout(function(){window.location='report_view.php'},1000);</script>";
    } else {
        $_SESSION['status'] = "sports_report Insertion Failed";
        $_SESSION['status_code'] = "error";
        echo "<script>setTimeout(function(){window.location='report_insert.php'},1000)</script>";
    }
}
?>
        ...
        <div class="content-wrapper">
            ...
            <div class="content-header">
                <div class="container-fluid">
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <h1 class="m-0">Add sports_report</h1>
                        </div><!-- /.col -->
                        <div class="col-sm-6">
                            <ol class="breadcrumb float-sm-right">
                                <li class="breadcrumb-item"><a href="../common/dashboard.php">Home</a></li>
                                <li class="breadcrumb-item active">Add sports_report</li>
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="card card-gmiu">
                                <div class="card-header">
                                    <h3 class="card-title">Add Activity report</h3>
                                </div>
                                <!-- /.card-header -->
                                <!-- form start -->
                                <form id="quickForm" method="POST" enctype="multipart/form-data">
                                    <div class="card-body">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="title" class="form-control" id="title_id" placeholder="Enter sports_report Report Title" required>
                                        </div>

                                        <div class="form-group">
                                            <label for="description">Description<span style="color: red;">*</span></label>
                                            <textarea name="description" class="ckeditor" id="description" placeholder="Enter Report Description" required></textarea>
                                        </div>
