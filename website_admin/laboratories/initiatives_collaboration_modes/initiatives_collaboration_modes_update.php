...
if (isset($_POST['submit'])) {
    ...
    if (!empty($image_upload['name'])) {
        ...
        if ($file_upload_status['status'] == 200) {
            ...
            echo "<script>setTimeout(function(){window.location='initiatives_collaboration_modes_edit.php?ic_id=$id'},1000)</script>";
            exit();
        }
    } else {
    ...
    if ($result) {
        $_SESSION['status'] = "Initiatives & Collaboration Modes Updated Successfully";
        $_SESSION['status_code'] = "success";
        echo "<script>setTimeout(function(){window.location='initiatives_collaboration_modes_view.php'},1000);</script>";
    } else {
        // Check for SQL errors
        $_SESSION['status'] = "Initiatives & Collaboration Modes Updation Failed: " . $stmt->error;
        $_SESSION['status_code'] = "error";
        echo "<script>setTimeout(function(){window.location='initiatives_collaboration_modes_edit.php?ic_id=$id'},1000)</script>";
    }
}
