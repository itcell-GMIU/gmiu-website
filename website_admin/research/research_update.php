...
if (isset($_POST['submit'])) {
    ...
    if (!empty($image_upload['name'])) {
        ...
        if ($file_upload_status['status'] == 200) {
            ...
            $stmt = $con->prepare("UPDATE `tbl_research` SET `title`=?, `img_name`=?, `description`=? WHERE id = ?");
            ...
            echo "<script>setTimeout(function(){window.location='research_edit.php?ic_id=$id'},1000)</script>";
            exit();
        }
    } else {
        // If no new image is uploaded, update the other fields without changing the existing image
        $stmt = $con->prepare("UPDATE `tbl_research` SET `title`=?, `description`=? WHERE id = ?");
    ...
    if ($result) {
        ...
        echo "<script>setTimeout(function(){window.location='research_view.php?id=$type'},1000);</script>";
    } else {
        // Check for SQL errors
        $_SESSION['status'] = "Updation Failed: " . $stmt->error;
        $_SESSION['status_code'] = "error";
        echo "<script>setTimeout(function(){window.location='research_edit.php?id=$type&ic_id=$id'},1000)</script>";
    }
}
?>