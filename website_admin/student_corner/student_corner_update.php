...
if ($role_id == 8) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $stmt = $con->prepare("UPDATE `tbl_std_corner` SET `faculty_id`=?, `level_id`=?, `program_id`=?, `sem`=? , `subject_code`=? , `subject_name`=? , `subject_short_name`=?,  `lectures`=?,  `tutorial`=?,  `practical`=?,  `credit`=? WHERE id = ?");
        $stmt->bind_param("iisisssiiiii", $faculty_id, $level_id, $program_id_str, $sem, $subject_code, $subject_name, $subject_short_name, $lectures, $tutorial, $practical, $credit, $id);
        $result = $stmt->execute();
        if ($result) {
