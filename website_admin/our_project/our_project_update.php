...
if ($role_id == 8) {
    if (isset($_POST["submit"])) {
        ...
        // Fetch hidden or existing faculty/program if not in form for role 8
        $faculty_id = isset($_POST['faculty_id']) ? mysqli_real_escape_string($con, $_POST['faculty_id']) : 0;
        $program_id = isset($_POST['program_id']) ? $_POST['program_id'] : [];
        $program_id_str = is_array($program_id) ? implode(',', array_map('intval', $program_id)) : mysqli_real_escape_string($con, $program_id);
        ...
        $stmt->bind_param("isissi", $faculty_id, $program_id_str, $level_id, $title1, $description,  $id);
        $result = $stmt->execute();
    ...
    if (isset($_POST["submit"])) {
        ...
        $level_id = validate_data($level_id);
        $title1 = validate_data($title1);
        // $program_id = validate_data($program_id); // Removed because it's an array
