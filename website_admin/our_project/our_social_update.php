...
// Report all errors and set display errors on
...
if ($role_id == 8) {
    ...
    if (isset($_POST["submit"])) {
        ...
        $id = mysqli_real_escape_string($con, $_POST['exp_id']);
        ...
        $stmt = $con->prepare("UPDATE `tbl_our_social` SET program_id = ?, title = ? WHERE id = ? ");
        $stmt->bind_param("ssi", $program_id, $title, $id);
        $result = $stmt->execute();
