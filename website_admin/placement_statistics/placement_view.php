                                ...
                                <table id="acedemic" class="dataTableLoad table table-bordered table-striped ">
                                    ...
                                    <tbody>
                                        ...
                                        $cmd = $con->prepare("SELECT 
                                                                    ...
                                                                    placement.student_name as placement_student_name, 
                                                                    placement.year as placement_year,
                                                                    placement.is_active as placement_is_active, 
                                                                    faculty.name as faculty_name,
                                                                    level.name as level_name,
                                                                    GROUP_CONCAT(program.name ORDER BY program.name SEPARATOR ', ') as program_name 
                                                                FROM tbl_placement as placement  
                                                                LEFT JOIN tbl_faculty faculty ON placement.faculty_id = faculty.id 
                                                                LEFT JOIN tbl_level level ON placement.level_id = level.id
                                                                LEFT JOIN tbl_program program ON FIND_IN_SET(program.id, placement.program_id) > 0 
                                                                WHERE placement.is_delete = ? and placement.faculty_id = ? and placement.level_id = ? and FIND_IN_SET(?, placement.program_id)
