...
if (strlen($_SESSION['student_id']) == 0 || $_SESSION['secretkey'] != "secret") {
  ...
  $student_id = $_SESSION['student_id'];
    ...
    stu.admission_quota as quota,
    faculty.shortname as faculty_shortname,
    faculty.name as faculty_name,    
    stu.account_office_approve_reject_date as account_office_approve_reject_date,
    stu.transaction_id as transaction_id,
    stu.payment_id as payment_id,
    fb.feedback_source,
    fb.referral_code,
    fb.admission_year,
    fb.method
  ...
  if ($result->num_rows == 1) {
    ...
    $feedback_source = !empty($row['feedback_source']) ? $row['feedback_source'] : "";
    $referral_code   = !empty($row['referral_code']) ? $row['referral_code'] : "";
    $admission_year  = !empty($row['admission_year']) ? $row['admission_year'] : "";
    $method = !empty($row['method']) ? $row['method'] : "";
