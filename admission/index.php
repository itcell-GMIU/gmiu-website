...
if (isset($_POST['register'])) {
    ...
    if (isset($_POST['captcha'])) {
        ...
        if ($captcha == $_SESSION['captcha_sum']) {
            ...
            $first_name = sanitize_name($_POST['first_name']);
            $middle_name = sanitize_name($_POST['middle_name']);
            $last_name = sanitize_name($_POST['last_name']);
                ...
                if (empty($password_error) && empty($first_name_error) && empty($middle_name_error) && empty($last_name_error) && empty($email_error) && empty($mobile_number_error) && empty($faculty_id_error) && empty($level_id_error) && empty($program_id_error)) {
                    ...
                    if ($stmt->execute()) {
                        $admission_student_id=$con->insert_id;
                         ...
                         // Prepare payload (send only required fields)
                        $apiPayload = json_encode([
                            ...
                            "local_student_id" => $admission_student_id
                        ]);

                        $apiUrl = "http://localhost/gmiu/admission/api-handler.php"; // CHANGE THIS
                        ...
                        if ($apiHttpCode == 200 || $apiHttpCode == 201) {
                            $apiStatus = "success";
                            // ✅ UPDATE is_erp = 1 (ONLY CHANGE ADDED)
                            $stmtErp = $con->prepare(
                                "UPDATE tbl_admission_student 
                                        SET is_erp = 1 
                                        WHERE id = ?"
                            );
                            $stmtErp->bind_param("i", $admission_student_id);
                            $stmtErp->execute();
                        } else {
                            $apiStatus = "failed";
                        }
                        ...
                        $inq_student_id = generateInquiryStudentId($con, $completed_study, $is_online);
                        ...
                        $result = $stmt->execute();
                        // ABOVE Insert into inquiry student table
        ...
        if ($ex->num_rows == 1) {
           ...
           exit(); */
             if ($admission_student_step == 5) {
                ...
                $user_agent = $_SERVER['HTTP_USER_AGENT']; // Get the user's browser/device info
                $referrer = $_SERVER['HTTP_REFERER']; // Get the referrer (where the user came from)
                $attempt_time = date('Y-m-d H:i:s'); // Current timestamp

                // Insert failed login attempt into the database
                $insert_failed_attempt = $con->prepare("INSERT INTO  tbl_admission_login_attempts (mobile_number, ip_address, user_agent, referrer, attempt_time, status) 
                                                  VALUES (?, ?, ?, ?, ?, ?)");
                $status = "success"; // Status for failed login
                $insert_failed_attempt->bind_param("ssssss", $mobile_number, $ip_address, $user_agent, $referrer, $attempt_time, $status);
                $insert_failed_attempt->execute();
            ...
            $insert_failed_attempt = $con->prepare("INSERT INTO  tbl_admission_login_attempts (mobile_number, ip_address, user_agent, referrer, attempt_time, status) 
                                          VALUES (?, ?, ?, ?, ?, ?)");
            $status = "failed"; // Status for failed login
            $insert_failed_attempt->bind_param("ssssss", $mobile_number, $ip_address, $user_agent, $referrer, $attempt_time, $status);
            $insert_failed_attempt->execute();
...
<meta name="google-site-verification" content="tr3GgfIHmTLyZRQzgyX5lFoWfxaDb18nhpX6_6s_e5Y" />
<link rel="canonical" href="https://gmiu.edu.in/gmiu/admission/" />
<link rel="preload" as="image" href="https://gmiu.edu.in/gmiu/website_assets/images/gmiulogo.webp" type="image/webp">
<link rel="shortcut icon" href="<?php echo $website_assets_url;?>images/favicon.ico" type="image/x-icon">

<!-- Google tag (gtag.js) -->
<script async src="https://www.googletagmanager.com/gtag/js?id=G-82N3HVJ6XT"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'G-82N3HVJ6XT');
</script>
...
})(window,document,'script','dataLayer','GTM-NMDXN8BH');</script>
<!-- End Google Tag Manager -->

        <?php $pageTitle = "Admission At Gyanmanjari Innovative University" ;
        //  $meta_description = 'Gyanmanjari Innovative University (GMIU), Bhavnagar invites students to apply for 2025 admissions across diverse programs including BTech, MTech, Diploma, BSc, MSc, BBA, MBA, and more. Discover our career-focused curriculum, experienced faculty, industry-driven training, and excellent placement support. Learn about eligibility criteria, fee structures, available scholarships, and step-by-step guidance for online application. Build your future with innovation, research, and holistic education at GMIU.';
        
         $meta_description = 'Apply to GMIU Bhavnagar for 2025 admissions in BTech, MTech, BSc, MBA & more. Enjoy expert faculty, industry training, scholarships, and placement support.';
...
<!--<div class="container" style="z-index:1; background-color: #fff; padding-top: 10px;">-->
                     <?php  //include '../website/post/tkm2k5.php'; ?>
<!--               </div>-->
...
<body class="home_version_02" onload="changeStyle(0)">
        ...
        <div class="overlay-bg">
            <div class="container">
                                    ...
                                    <form class="signup" method="POST" id="setting">
                                        <div class="form-row" style="margin-top : 20px; ">
                                            <div class="form-group col-md-6">
                                                <label class="colorcode">Student Name</label>
                                                <span class="text-danger">*</span>
                                            ...
                                            <div class="form-group col-md-6">
                                                ...
                                                <select class="form-control tp" id="faculty_id" name="faculty_id"
                                                    ...
                                                    <?php
                                                    $query = "SELECT * FROM tbl_faculty WHERE is_active = 1 and is_delete=0 and id != '19'";
                                            ...
                                            <div class="form-group col-md-6">
                                             ...
                                             <div class="form-group col-md-6">
                                                <label class="colorcode">Your Last Exam:</label>
                                                <span class="text-danger">*</span>
                                                <select class="form-control tp" id="completed_study"
                                                    name="completed_study" required>
                                                    <option value="">--Please select--</option>
                ...
                <div style="color:white; font-size: medium;" class="col-sm-7">
                    <p style="color:white;">Apply Online for Confirm</p>
                    <h1 style="color: red; background-color: #fff;text-align: center;border-radius: 20px;">Admission 2025-26</h1>
                    <div class="row">
                    ...
                    <div class="contDiv">
                        ...
                        <ul style="columns:2;">
                              ...
                              <a style="color:white;" href="https://gmiu.edu.in/gmiu/website/faculty/faculty-of-engineering-amp-technology-diploma" target="_blank">
                                     <li>   INSTITUTE OF ENGINEERING & TECHNOLOGY(DIPLOMA) </li>
                              </a>
                            <?php
                            $query = "SELECT id as faculty_id,name as faculty_name, faculty_slug FROM tbl_faculty as faculty WHERE is_active = 1 and is_delete=0";
                            $result = $con->query($query);
                            if ($result->num_rows > 0) {
                                while ($row = $result->fetch_assoc()) {
                                     $faculty_name = ($row['faculty_id'] == 1) ? strtoupper($row['faculty_name']) . " (DEGREE)" : strtoupper($row['faculty_name']);
                            ?>
                            <a style="color:white;" href="<?php echo $base_url_website_faculty?><?php echo $row['faculty_slug']; ?>">
                    ...
                    <div class="col-sm-5">
                       <div class="wrapper" id="wrp" style="padding:0px; text-align:center;">
                       <h4><a href="<?php echo $website_assets_url; ?>images/2025-2026.jpg" style="color:black;" target="_blank">
                       Overall Program
                       </a></h4>
                       </div>
                      </div>
    ...
    <section style="padding: 20px; padding-bottom: 50px; display: block;" class="howTo">
        <div style="font-size: large;" class="container">
            <h3 style="font-size: 40px; padding-top: 50px;line-height: 1.1;">How to <strong>Apply</strong>/<strong>Confirm</strong> Admission?</h3>
                ...
                <tbody style="font-size: 200%;">
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <p>
                               <a href="https://gmiu.edu.in/gmiu/website_assets/gmiu_doc/online-registration1.pdf" target="_blank">Click Here </a>
                            </p>
                        </td>
                    </tr>
                </tbody>
            ...
            <div class="faculty-track">
                <div class="faculty-slide">
                    <a href="https://gmiu.edu.in/gmiu/website/faculty/faculty-of-engineering-amp-technology-diploma" target="_blank">
                        <img src="https://gmiu.edu.in/gmiu/website_assets/images/DEPARTMENT-THUMBNAIL/diploma.jpg" alt="Diploma Faculty" class="faculty-thumb">
                ...
                ?>
                    <div class="faculty-slide">
                        <a href="<?php echo $base_url_website_faculty . $row['faculty_slug']; ?>" target="_blank">
                            <img src="<?php echo $thumbnail; ?>" alt="<?php echo strtoupper($row['faculty_name']); ?>" class="faculty-thumb">
                            <div class="faculty-name"> <?php echo strtoupper($row['faculty_name']); ?> </div>
                        </a>
                    </div>
                <?php
                }
    ...
    <section class="RecAndWhy">
        <div class="container">
            <div class="recruiters">
                <h3>Major <strong>Placements</strong></h3>
                <div class="carouselDiv">
                    <?php for ($i = 1; $i <= 16; $i++) { ?>
                    <div class="slide">
                        <div class="card">
                            <div class="img"
                                style="background-image: url(https://www.gmiu.edu.in/gmiu/website_assets/images/associates/p<?php echo $i; ?>.jpg)">
                            </div>
                        </div>
                    </div>
                    <?php } ?>
