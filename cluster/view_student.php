...
if (isset($_POST['comment_btn'])) {
    ...
    $update->bind_param("si", $comment, $stu_id);
    if ($update->execute()) {
                    ...
                    <div class="row mb-2">
                        <div class="col-sm-6">
                            <h1 class="m-0">View <?php
                                                    if ($url_for == 'all') {
                                                        echo " Total ";
                                                    } elseif ($url_for == 'pending_student') {
                                                        echo " Pending";
                                                    } elseif ($url_for == 'approved_student') {
                                                        echo " Approved";
                                                    } elseif ($url_for == 'rejected_student') {
                                                        echo " Rejected";
                                                    }
                                                    ?>
                                Student
                                    ...
                                    <div class="form-row">
                                        <div class="col-md-4">
                                            <div class="form-label-group">
                                                ...
                                                <select name="url_faculty_id" id="faculty_id"
                                                    class="browser-default custom-select"
                                                    style="color:black; border-color:#325d88; border-width:2px"
                                                    required>
                                                    <option value="">---Select Faculty---</option>
                                                    <?php
                                                    $query = "SELECT * FROM tbl_faculty WHERE `id` IN ($list_faculty_id);";
                                                    $result = $con->query($query);
                                                    if ($result->num_rows > 0) {
                                            ...
                                            <div class="form-label-group">
                                                <select name="url_level_id" id="level_id"
                                                    ...
                                                    <option value="">---Select Level---</option>
                                                     <?php
                                                    $query = "SELECT * FROM tbl_level WHERE `id` IN ($list_level_id);";
                                                    $result = $con->query($query);
                                                    if ($result->num_rows > 0) {
                                                        while ($row = $result->fetch_assoc()) {
                                                            echo '<option value=' . $row['id'] . '>' . $row['name'] . '</option>';
                                ...
                                <table id="acedemic" class="dataTableLoad table table-bordered table-striped ">
                                    ...
                                    <tbody>
                                        <?php
                                                ...
                                                WHERE 
                                                    stu.faculty_id IN ($list_faculty_id) 
                                                     AND stu.program_id IN ($list_program_ids)
                                                     AND stu.level_id IN ($list_level_id)
                                                     AND stu.is_active = 1 
                                                     AND stu.is_delete = 0
                                                ";
                                                ...
                                                if ($url_for != "") {
                                                    if ($url_for == "approved_student") {
                                                        $cmd .= " AND stu.status = 'approved'";
                                                    } elseif ($url_for == "rejected_student") {
                                                        $cmd .= " AND stu.status = 'rejected'";
                                                    } elseif ($url_for == "pending_student") {
                                                        $cmd .= " AND stu.status != 'approved' AND stu.status != 'rejected'";
                                                    }
                                                }
                                                ...
                                                if ($url_program_id != "") {
                                                    $cmd .= " AND stu.program_id = '$url_program_id'";
                                                }
                                        ...
                                        <tr align="center">
                                            ...
                                            <script>
                                            load_student_data('<?php echo $sts; ?>', '<?php echo $stu_id; ?>');
                                            </script>
                                            <td scope="row"><button id="approved<?php echo $stu_id ?>"
                                                    data-student_id="<?php echo $stu_id ?>" data-student_email="<?php echo $stu_email ?>" data-name="approved"
                                                    class="btn btn-success approve_reject"><i
                                                        class="fas fa-check"></i></button>
                                                <button id="rejected<?php echo $stu_id ?>"
                                                    data-student_id="<?php echo $stu_id ?>" data-student_email="<?php echo $stu_email ?>" data-name="rejected"
                                            ...
                                            <td scope="row"><?php if ($stu_account_office_status == "approved") {
                                                                        ?>
                                                <span
                                                    class="badge badge-success"><?php echo $stu_account_office_status ?></span>
                                                <?php } elseif ($stu_account_office_status == "reject") { ?>
                                                <span
                                                    class="badge badge-danger"><?php echo $stu_account_office_status ?></span>

                                                <?php   } else { ?>
                                                <span
                                                    class="badge badge-warning"><?php echo $stu_account_office_status ?></span>
                                            ...
                                            <td scope="row">
                                                <form action="" method="POST">
                                                    ...
                                                    <button type="submit" name="comment_btn" value="comment"
