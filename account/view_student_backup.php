                                ...
                                <table id="acedemic" class="dataTableLoad table table-bordered table-striped">
                                    <thead>
                                        <tr align="center">
                                            ...
                                            <th scope="row" style="color: black;"><b>Book Bank Status</b></th>
                                            <th scope="row" style="color: black;"><b>stationery Status</b></th>
                                            <th scope="row" style="color: black;"><b>Pending Fees</b></th>
                                    ...
                                    <tbody>
                                        ...
                                        if ($rows > 0) {
                                            while ($row = $result->fetch_assoc()) {
                                                ...
                                                $bookbank = $row['bookbank_fees_status'];
                                                if ($bookbank == 0) {
                                                    $status = "Pending";
                                                } else if ($bookbank == 1) {
                                                    $status = "Completed";
                                        ...
                                        ?>
                                                <tr align="center">
                                                    ...
                                                    <td scope="row"><?php if ($stu_payment_status == "success") {
                                                                    ?>
                                                            <a href="view_receipt.php?stu_id=<?php echo $stu_id; ?>" style="color:blue;" target="_blank"><button type="button" class="btn btn-success">View</button></a>
                                                        <?php }
                                                    ...
                                                    <td scope="row"><button id="approved<?php echo $stu_id ?>" <?php $stu_payment_status == "success"
                                                                                                                ?> data-name="approved" data-student_id="<?php echo $stu_id ?>" class="btn btn-success approve_reject"><i class="fas fa-check"></i></button>
                                                        <button id="rejected<?php echo $stu_id ?>" data-student_id="<?php echo $stu_id ?>" data-name="rejected" class="btn btn-danger approve_reject"><i class="fas fa-times"></i>
                                                        </button>
                                                    </td>
                                                    ...
                                                    <td scope="row"><?php if ($stu_payment_status == "success") {
                                                                    ?>
                                                            <span class="badge badge-success"><?php echo $stu_payment_status ?></span>
                                                        <?php } else { ?>
                                                            <span class="badge badge-danger"><?php echo $stu_payment_status ?></span>

                                                        <?php   }
                                                        ?>
                                                    </td>
                                                    <td scope="row"><span id="status_lable_<?php echo $stu_id ?>" class="badge"></span></td>
                                                    <td scope="row"><?php echo $status; ?></td>
                                                    <td scope="row"><?php echo $status2; ?></td>

                                                    <td scope="row"><?php
                                                                    $cmd2 = "SELECT `regular`, `genius`, `honors`, `international`, `minor` ,`blended_mode`,`token` ,`token_minor` , `token_genius` ,`token_international` , `token_honors` ,`token_blended_mode` FROM `tbl_program` WHERE `id`=? AND `faculty_id`=? AND `level_id`=?";
                                                                    $stmt2 = $con->prepare($cmd2);
                                                                    $stmt2->bind_param("iii", $stu_program_id, $stu_faculty_id, $stu_level_id);
                                                                    $stmt2->execute();
                                                                    $result2 = $stmt2->get_result();

                                                                    if ($result2->num_rows != 0) {
                                                                        $row = $result2->fetch_assoc(); // Fetch a row from the result set

                                                                        $fees_regular_year = $row['regular'];
                                                                        $fees_genius_year = $row['genius'];
                                                                        $fees_honors_year = $row['honors'];
                                                                        $fees_international_year = $row['international'];
                                                                        $fees_minor_year = $row['minor'];
                                                                        $blended_mode = $row['blended_mode'];
                                                                        $token = $row['token'];
                                                                        $token_minor = $row['token_minor'];
                                                                        $token_genius = $row['token_genius'];
                                                                        $token_international = $row['token_international'];
                                                                        $token_honors = $row['token_honors'];
                                                                        $token_blended_mode = $row['token_blended_mode'];
                                                                        $fees_regular_sem = $fees_regular_year / 2;
                                                                        $fees_genius_sem = $fees_genius_year / 2;
                                                                        $fees_honors_sem = $fees_honors_year / 2;
                                                                        $fees_international_sem = $fees_international_year / 2;
                                                                        $fees_minor_sem = $fees_minor_year / 2;

                                                                        

                                                                        if ($mode == "regular") {
                                                                            echo $fees_regular_sem -   $stu_token_amount;
                                                                        } else if ($mode == "genius") {
                                                                            echo $fees_genius_sem -  $stu_token_amount  ;   
                                                                        } else if ($mode == "honors") {
                                                                            echo $fees_honors_sem -  $stu_token_amount ;
                                                                        } else if ($mode == "international") {
                                                                            echo $fees_international_sem -  $stu_token_amount;
                                                                        } else if ($mode == "minor") {
                                                                            echo $fees_minor_sem -  $stu_token_amount;
                                                                        }
                                                                    }
                                                                    ?></td>
                                                    ...
                                                    <td scope="row">
                                                        <form action="" method="POST">
                                                            <div class="form-group">
                                                                ...
                                                                <textarea font-size: 18px; class="form-control" rows="2" id="account_office_comment" name="account_office_comment" required></textarea>
                                                            </div>
                                                            <button type="submit" name="comment_btn" value="account_office_comment" class="btn btn-block btn-primary btn-xs">Submit</button>
                                                        </form>
                                                    </td>
