...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_account_status') {
    ...
    $stmt = $con->prepare("UPDATE tbl_cancellation_requests SET account_status = ?, account_remark = ?, account_at = NOW(), updated_at = NOW() WHERE id = ?");
...
ON
    p.id = tas.program_id
WHERE
    tcr.is_active = 1 AND tcr.is_delete = 0 AND tcr.hod_status = ? AND tcr.cluster_status = ?
");
$hod_status = "approved";
$cluster_status = "approved";
$stmt->bind_param("ss", $hod_status, $cluster_status);
$stmt->execute();
                                            ...
                                            <tbody>
                                                ...
                                                if ($result->num_rows > 0) {
                                                    while ($row = $result->fetch_assoc()) {
                                                        ...
                                                        if ($status === 'pending') {
                                                            echo "<button class='btn btn-success btn-sm change-account-status' data-id='" . $row['tcr_id'] . "' data-status='Approved' title='Approve'>
                                                              <i class='fa fa-check'></i></button>";
                                                            echo "<button class='btn btn-danger btn-sm change-account-status' data-id='" . $row['tcr_id'] . "' data-status='Rejected' title='Reject'>
                                                              <i class='fa fa-times'></i></button>";
                                                        } elseif ($row['eligibility_status'] == 'pending') {
                                                            if ($status === 'rejected') {
                                                                echo "<button class='btn btn-success btn-sm change-account-status' data-id='" . $row['tcr_id'] . "' data-status='Approved' title='Approve'>
                                                               <i class='fa fa-check'></i> </button>";
