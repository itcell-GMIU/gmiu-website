...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_account_status') {
    ...
    $stmt = $con->prepare("UPDATE tbl_branch_transfer_requests SET account_approval = ?, a_remark = ?, updated_at = NOW() WHERE id = ?");
...
$stmt = $con->prepare("
    ...
    WHERE btr.is_delete = 0 and btr.clusteradmin_approval = 'approved' and btr.hod_approval = 'approved' and btr.program_id IS NOT NULL
...
<body class="hold-transition sidebar-mini layout-fixed">
                                            ...
                                            <tbody>
                                                ...
                                                if ($result->num_rows > 0) {
                                                    while ($row = $result->fetch_assoc()) {
                                                        ...
                                                        echo "<td>";
                                                        $status = $row['account_approval'];
                                                        $ss_status = $row['student_section_approval'];


                                                        if ($status === 'pending') {
                                                            echo "<button class='btn btn-success btn-sm change-account-status' data-id='" . $row['b_id'] . "' data-status='Approved' title='Approve'>
                                                                <i class='fa fa-check'></i>
                                                            </button>";
                                                            echo "<button class='btn btn-danger btn-sm change-account-status' data-id='" . $row['b_id'] . "' data-status='Rejected' title='Reject'>
    ...
    <script>
        $(document).on('click', '.change-account-status', function() {
