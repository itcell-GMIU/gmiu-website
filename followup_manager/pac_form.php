...
<?php
// $admy = $_GET['admy'];
// $query = "SELECT COUNT(*) AS total FROM tbl_pac_form WHERE admy = '$admy' and is_active = 1 AND is_delete = 0";
...
if ($admy !== 'N') {
    // ✅ Case 1: Normal admission
    $query = "SELECT COUNT(*) AS total 
              FROM tbl_pac_form 
              WHERE admy = '$admy' AND is_active = 1 AND is_delete = 0";
    $data = $con->query($query);
    if ($data) {
        $count = $data->fetch_assoc();
        $currentFormNo = $count['total'] + 1;
    } else {
        $currentFormNo = 1; // fallback
    }

} else {
    // ✅ Case 2: When admy = 'N' (special numbering format)
    // $query = "SELECT formno 
    //           FROM tbl_pac_form 
    //           WHERE formno LIKE '%/%' 
    //           ORDER BY pacid DESC 
    ...
    $query = "SELECT formno
          ...
          ORDER BY pacid DESC
          LIMIT 1";
              // get a few recent to check prefix
    $data = $con->query($query);

    if ($data && $data->num_rows > 0) {
    
        $row = $data->fetch_assoc();
        $lastFormNo = $row['formno']; // Example: "2025 / 1897"
    
        $parts = explode('/', $lastFormNo);
        $lastNumber = (int)trim($parts[1]);
    
        $nextNumber = $lastNumber + 1;
      
        $currentFormNo =  $nextNumber;
...
$stmt = $con->prepare("SELECT 
    CONCAT_WS(' ', COALESCE(stu.first_name, ''), COALESCE(stu.middle_name, ''), COALESCE(stu.last_name, '')) AS full_name,
    stu.email,
    stu.mobile_number,
    stu.dob,
    stu.faculty_id,
...
LEFT JOIN tbl_admission_feedback AS fb 
    ON stu.id = fb.student_id 
    AND fb.is_active = 1 
    AND fb.is_delete = 0
WHERE stu.id = ?;");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $admy = $_POST['admy'];
     ...
     // for the old faculty, level and program 
    $old_faculty = $_POST['old_faculty'];
    $old_level = $_POST['old_level'];
    $old_program = $_POST['old_program'];
    ...
    $sql = "INSERT INTO tbl_pac_form (
               ...
               formno, remarks, level2, minor, honour, is_edited, admy
           ) VALUES (
               ?, ?, ?, ?, ?, 
               ?, ?, ?, ?, ?, 
               ?, ?, ?, ?, ?, 
    ...
    if ($stmt = $con->prepare($sql)) {
        ...
        if ($stmt->execute()) {
              ...
              if (($faculty_id != $old_faculty) || ($level_id != $old_level) || ($program_id != $old_program)) {
                $changes = [];
                if ($faculty_id != $old_faculty)
                    $changes['previous_faculty_id'] = $old_faculty;
                if ($level_id != $old_level)
                    $changes['previous_level_id'] = $old_level;
                if ($program_id != $old_program)
                    $changes['previous_program_id'] = $old_program;

                $json = $con->real_escape_string(json_encode($changes));

                $con->query("UPDATE tbl_admission_student SET 
                    faculty_id = $faculty_id, 
                    level_id = $level_id, 
                                ...
                                <form id="quickForm" method="POST" action="#" class="needs-validation" novalidate>
                                    <div class="card-body">
                                        ...
                                        <div class="row">
                                            ...
                                            <?php
                                            $paddedNo = str_pad($currentFormNo, 4, '0', STR_PAD_LEFT);
                                            $formno_display = '';
                                            if ($admy === 'N') {
                                                $formno_display = "2025 / $paddedNo";
                                            } elseif ($admy === 'B') {
                                                $formno_display = "B2026 / $paddedNo";
                                            } elseif ($admy === 'R') {
                                                $formno_display = "R2026 / $paddedNo";
                                            } else {
                                                $formno_display = "2025 / $paddedNo"; // fallback
                                            }
                                            ?>
                                            
                                            <!-- Visible disabled input -->
                                            <input type="text" class="form-control" id="formno_display" value="<?php echo htmlspecialchars($formno_display); ?>"
                                                required disabled>
                                            
                                            <!-- Hidden input for form submission -->
                                            <input type="text" class="form-control" id="formno" name="formno"
                                                value="<?php echo htmlspecialchars($formno_display); ?>" required hidden>

                                                <input type="text" name="admy" value="<?php echo $admy; ?>" required hidden>
                                            </div>
                                        </div>
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="form-group col-md-4">
                                             ...
                                             <!-- Hidden fields for the old faculty, level and program  -->
                                             <input type="text" name="old_faculty" value="<?php echo $facultyID;?>" hidden>
                                             <input type="text" name="old_level" value="<?php echo $levelID;?>" hidden>
                                             <input type="text" name="old_program" value="<?php echo $programID;?>" hidden>
                                        </div>
