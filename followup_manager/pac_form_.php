...
$stmt->bind_param("i", $id); // Assuming stu_id is an integer
$stmt->execute();
...
<body class="hold-transition sidebar-mini layout-fixed">
                                ...
                                <form id="quickForm" method="POST" action="#" class="needs-validation" novalidate>
                                    <div class="card-body">
                                        ...
                                        <div class="row">
                                            <!-- Select Faculty -->
                                            <div class="form-group col-md-6">
                                                <label class="control-label">Select Faculty<span style="color: red;">*</span>
                                                </label>
                                        ...
                                        <div class="row">
                                            ...
                                            <!-- <div class="form-group col-md-6">
                                                ...
                                                <select class="form-control" id="branchSpecialization" name="branchSpecialization" required onchange="fetchBranchDetails(this.value)">
                                                    <option value="">Select Branch/Specialization</option>
                                                    
                                                    <?php
                                                    // $queryBranch = "SELECT 
                                                    //                     p.branch_code, 
                                                    //                     CONCAT(p.name, ' (', l.name, ')') AS program_name
                                                    //                 FROM tbl_program p
                                                    ...
                                                    //         echo '<option value="' . htmlspecialchars($rowBranch['branch_code']) . '">' . htmlspecialchars($rowBranch['branch_code'] .' - '. $rowBranch['program_name']) . '</option>';
                                                    //     }
                                                    // }
                                                    ?>
                                                </select>
    ...
    <script>
        ...
        function load_level() {
            ...
            var path = './include/';
            var faculty_id = <?php echo $facultyID; ?>;
        ...
        function load_program() {
            ...
            var faculty_id = <?php echo $facultyID; ?>;
            var level_id = <?php echo $levelID; ?>;
            var program_id = <?php echo $programID; ?>; // Default program (if exists)
            var api_for = "dashboard";
                ...
                .then(data => {
                    ...
                    if (data.error) {
                        ...
                        if (tokenInput && remainingInput) {
                            ...
                            // const levelCheck = document.getElementById('level_id').value;
                            // if(levelCheck == 1 || levelCheck == 2){
                            //     remainingFees = remainingFees - 20000;
                            // }else if(levelCheck == 5){
                            //     remainingFees = remainingFees - 10000;
                            // }
        ...
        $('#level_id').on('change', function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $('#branchSpecialization').html(data);
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            const path = "./include/"; // Change this to your actual path
                    ...
                    .then(data => {
                        if (data.options) {
                            ...
                            data.options.forEach(option => {
                                optionsHtml += `<option value="${option.id}">${option.text}</option>`;
                            });

                            $('#' + selectId).html(optionsHtml); // Update the correct select element
                        } else {
            ...
            document.getElementById("Minor").addEventListener("change", function() {
                if (this.checked) {
                    ...
                    sendRequest("minor.php", "minor", faculty_id, level_id);
                } else {
                    document.getElementById('minor-select').classList.add("d-none");
                    $('#minor').html('<option value="">--Please select--</option>'); // Clear options if unchecked
                }
            });
            ...
            document.getElementById("Honour").addEventListener("change", function() {
                if (this.checked) {
                    ...
                    let faculty_id = document.getElementById('faculty_id').value;
                    let level_id = document.getElementById('level_id').value;
                    sendRequest("honour.php", "honour", faculty_id, level_id);
                } else {
                    document.getElementById('honour-select').classList.add("d-none");
                    $('#honour').html('<option value="">--Please select--</option>'); // Clear options if unchecked
                }
            });
    ...
    <script>
        // Track the last applied minor and honour fees so we can remove them if changed
        let lastMinorFee = 0;
        let lastHonourFee = 0;

        // Get references to the inputs showing fee values
        const remainingInput = document.getElementById('remainingFees');
        const remainingFeesShowOnly = document.getElementById('remainingFeesShowOnly');
        ...
        document.getElementById('minor').addEventListener('change', function() {
            ...
            // console.log("Selected minor program_id:", selectedValue);

            // Step 1: Remove previously added minor fee
            if (lastMinorFee > 0) {
                let currentRemaining = Number(remainingInput.value);
                currentRemaining -= lastMinorFee; // subtract the old minor fee
                remainingInput.value = currentRemaining;

                // Update the displayed fee breakdown string
                let showFees = remainingFeesShowOnly.value;
                const feePattern = new RegExp(`\\s?\\+\\s?${lastMinorFee}`);
                showFees = showFees.replace(feePattern, '');
                remainingFeesShowOnly.value = showFees;

                // Reset the tracker
                lastMinorFee = 0;
            }

            // Step 2: Add the newly selected minor fee
            if (selectedValue) {
                fetch('./include/minor_honour_fees_add.php', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json'
                        },
                    ...
                    .then(response => response.json())
                    .then(data => {
                        const minorFee = Number(data.data.sem1);
                        lastMinorFee = minorFee; // Save for future removal

                        // Add to remaining
                        let newRemaining = Number(remainingInput.value);
                        newRemaining += minorFee;
                        remainingInput.value = newRemaining;

                        // Show fee addition visually
                        let showFees = remainingFeesShowOnly.value;
                        showFees += ' + ' + minorFee;
                        remainingFeesShowOnly.value = showFees;
                    })
                    .catch(error => {
                        // console.error('Minor fetch error:', error);
                    });
            }
        });

        // When the "Minor" checkbox is unchecked, remove any applied minor fee
        document.getElementById("Minor").addEventListener("change", function() {
            if (!(this.checked)) {
                if (lastMinorFee > 0) {
                    let currentRemaining = Number(remainingInput.value);
                    currentRemaining -= lastMinorFee;
                    remainingInput.value = currentRemaining;
        ...
        // ========================
        // Handle HONOUR selection
        // ========================
        document.getElementById('honour').addEventListener('change', function() {
            const selectedValue = this.value;
            // console.log("Selected honour program_id:", selectedValue);

            // Step 1: Remove previously added honour fee
            if (lastHonourFee > 0) {
                let currentRemaining = Number(remainingInput.value);
                currentRemaining -= lastHonourFee;
                remainingInput.value = currentRemaining;

                let showFees = remainingFeesShowOnly.value;
                const feePattern = new RegExp(`\\s?\\+\\s?${lastHonourFee}`);
                showFees = showFees.replace(feePattern, '');
                remainingFeesShowOnly.value = showFees;
            ...
            // Step 2: Add the newly selected honour fee
            if (selectedValue) {
                fetch('./include/minor_honour_fees_add.php', { // Reuse same API
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json'
                        },
                    ...
                    .then(response => response.json())
                    .then(data => {
                        const honourFee = Number(data.data.sem1);
                        lastHonourFee = honourFee; // Save for future removal

                        // Add to remaining
                        let newRemaining = Number(remainingInput.value);
                        newRemaining += honourFee;
                        remainingInput.value = newRemaining;
                    ...
                    .catch(error => {
                        // console.error('Honour fetch error:', error);
                    });
            }
        });

        // When the "Honour" checkbox is unchecked, remove any applied honour fee
        document.getElementById("Honour").addEventListener("change", function() {
            if (!(this.checked)) {
                if (lastHonourFee > 0) {
                    let currentRemaining = Number(remainingInput.value);
...
<!-- on change of the gb changing the value  -->
<script> 
  const checkbox = document.getElementById("typeGB");
  const remainingFeesInput = document.getElementById("remainingFees");
  const remainingFeesShowOnlyInput = document.getElementById("remainingFeesShowOnly");

  // ✅ Declare backup variables outside but initialize inside only once
  let originalRemainingFees = null;
  let originalRemainingFeesShowOnly = null;

  checkbox.addEventListener("change", function () {
    // ✅ Read level dynamically
    ...
    // ✅ Save original values only once when checkbox is first checked
    if (originalRemainingFees === null || originalRemainingFeesShowOnly === null) {
      originalRemainingFees = parseInt(remainingFeesInput.value) || 0;
      originalRemainingFeesShowOnly = parseInt(remainingFeesShowOnlyInput.value) || 0;
    }

    if (checkbox.checked) {
      let newRemainingFees = originalRemainingFees;
      let newRemainingFeesShowOnly = originalRemainingFeesShowOnly;

      if (facultyCheck === 1 && (levelCheck === 1 || levelCheck === 2)) {
        newRemainingFees -= 20000;
        newRemainingFeesShowOnly -= 20000;
      } else if (facultyCheck === 1 && (levelCheck === 5)) {
        newRemainingFees -= 10000;
        newRemainingFeesShowOnly -= 10000;
      }

      // ✅ Update UI
      remainingFeesInput.value = newRemainingFees.toString();
      remainingFeesShowOnlyInput.value = newRemainingFeesShowOnly.toString();

    } else {
      // ✅ Restore original values
      remainingFeesInput.value = originalRemainingFees.toString();
      remainingFeesShowOnlyInput.value = originalRemainingFeesShowOnly.toString();
    }
  });
</script>
