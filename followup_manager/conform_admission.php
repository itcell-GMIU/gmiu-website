...
include 'phpmailer_smtp/send_mail.php';
...
if (isset($_POST['submit'])) {
    ...
    if (mysqli_num_rows($mobileResult) > 0) {
        ...
        if (empty($password_error) && empty($first_name_error) && empty($middle_name_error) && empty($last_name_error) && empty($email_error) && empty($mobile_number_error) && empty($faculty_id_error) && empty($level_id_error) && empty($program_id_error)) {
            ...
            if ($stmt->execute()) {
                $confirm = 1;
                $stmt1 = $con->prepare("UPDATE `tbl_inquiry_student` SET is_admission_confirm = ? , counselor_id = ?, mobile_number = ?, email = ?  WHERE id = ? ");
                $stmt1->bind_param("iissi", $confirm,$staff_id, $mobile_number, $email, $inq_id);
               ...
               $to = $email;
                ...
                $send_mail_error = smtp_mailer($to, $subject, $message);
                if ($send_mail_error === true) {
                    $mail_status = "success";
                } else {
                    $mail_status = $send_mail_error; // Store the error message
                }
                ...
                $stmt5->execute();
                if($send_mail_error === true){
                    $_SESSION['status'] = "Registered Successfully,Password Will Sended on your registered Email Address.";
                }else{
                    $_SESSION['status'] = "Registered Successfully,Password Sending Failed to Email Address !!!";
                }
                $_SESSION['status_code'] = "success";
                echo "<script>setTimeout(function(){window.location='student_list.php'},2000)</script>";
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="row">
                                            ...
                                            <div class="form-group col-sm-3">
                                                ...
                                                <input type="number" name="mobile_number" value="<?php echo $mobile_number1 ?>" class="form-control" placeholder="Enter Mobile Number" required>
                                            </div>
                                            <div class="form-group col-sm-3">
                                            ...
                                            <div class="form-group col-sm-3">
                                                ...
                                                <input type="email" name="email" value="<?php echo $email1 ?>" class="form-control" placeholder="Enter Email" required>
                                            </div>
                                            <div class="form-group col-sm-3">
