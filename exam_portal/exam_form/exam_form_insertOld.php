...
if (isset($_POST['submit'])) {
    ...
    if ($result1) {
        ...
        // $subject_codes_array = json_decode($subject_codes_json, true);
        // if (is_array($subject_codes_array)) {
        //     foreach ($subject_codes_array as $subject_code) {
        //         $cmd2 = $con->prepare("SELECT `subject_name` FROM `tbl_std_corner` WHERE `subject_code`= ?");
        //         $cmd2->bind_param("s", $subject_code);
        //         $cmd2->execute();
        //         $result2 = $cmd2->get_result();
        //         while ($row2 = $result2->fetch_assoc()) {
        //             $sub_name = $row2['subject_name'];
        //         }
        //         $stmt44 = $con->prepare("INSERT INTO tbl_exam_subjects (exam_id, subject_code, subject_name) VALUES (?,?,?)");
        //         $stmt44->bind_param("iss", $new_id, $subject_code, $sub_name);
        //         $stmt44->execute();
        //     }
        // }
