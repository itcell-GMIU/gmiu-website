...
if (isset($_POST['btn-login'])) {
    ...
    if ($role == 51) {
        ...
        $cmd->execute();
    } elseif ($role == 52 || $role == 53) {
        $cmd = $con->prepare("SELECT role_id, `id` as staff_id, `email` as staff_email from tbl_exam_staff  WHERE email=? AND password= ? AND role_id= ? AND is_active=1 AND is_delete=0");
        $cmd->bind_param("ssi", $email, $password, $role);
        $cmd->execute();
    }
                ...
                <form action="" method="POST">
                    ...
                    <div class="input-group mb-3">
                        ...
                        <select name="role" class="browser-default custom-select" required>
                            ...
                            <?php
                            $query = "SELECT id,name FROM tbl_role WHERE is_active = 1 and is_delete=0 and id IN (51,52,53) ";
