...
if (isset($_GET['erNo']) && isset($_GET['exam_id']) && isset($_GET['token'])) {
    $token = $_GET['token'];
    $erNo = $_GET['erNo'];
    $exam_id = $_GET['exam_id'];
    $std_image = $_GET['image'];
    $std_signature = $_GET['signature'];
...
<body>
    <div class="container">
        <!-- Receipt Container -->
        <?php
        $cmd = $con->prepare("SELECT `id`, `faculty_id`, `level_id`, `program_id`, `semester`, `type`, `subject_code`, `session`, `start_date`, `end_date`, `year`, `subject_fee` FROM `tbl_exam_form` WHERE `is_active` = 1 AND `id` = ?");
        ...
        ?>
            <div class="receipt-container">
                <div class="row">
                    <div class="col-2 text-center border-dark border-left border-top border-bottom">
                        <img src="../../website_assets/images/logo-single.jpg" alt="Logo" class="logo mt-1 mb-1 ml-auto" style="z-index: 1;">
                    </div>
                ...
                <div class="row">
                    ...
                    <div class="col-12 text-center border-left border-right border-bottom p-h border-dark" style="height: 85px;">
                        <div class="row m-2">
                ...
                <div class="row">
                    ...
                    <div class="col-2 border-right text-center border-dark">
                        <img src="<?= $std_image ?>" alt="Photo" class="m-1" style="height: 120px; width: 100%;">
                        <div class="row border-top border-dark">
                            <img src="<?= $std_signature ?>" alt="Signature" class="m-1" style="height: 50px; width: 100%;">
                        </div>
                    </div>
                            ...
                            <tbody>
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <?php
                                        $cmd2 = $con->prepare("SELECT `subject_code`, `subject_name` FROM `tbl_student_subjects` WHERE `enrollnment_no`= ? AND `semester`= ? AND is_active = 1");
                                        $cmd2->bind_param("si", $er_no, $semester_ex);
                                        $cmd2->execute();
                                        $result2 = $cmd2->get_result();

                                        // Create an array to store subject codes and their corresponding timetable dates
                                        $subjectsWithDates = array();
                                        ...
                                        while ($row2 = $result2->fetch_assoc()) {
                                            ...
                                            foreach ($subjectArray as $subject_code) {
                                                $examtt = $con->prepare("SELECT `subject_code`, `date` FROM `tbl_exam_timetable` WHERE `subject_code`= ? AND `sub_type`= 'Theory' and exam_id = ?");
                                                $examtt->bind_param("si", $subject_code, $exam_id);
                                                $examtt->execute();
                                                $examttres = $examtt->get_result();

                                                while ($rowtt = $examttres->fetch_assoc()) {
                                                    $subject_c = $rowtt['subject_code'];
                                                    $timetable_date = $rowtt['date'];
                                                    // Store subject code and timetable date in the array
                                                    $subjectsWithDates[$subject_c] = $timetable_date;
                                                }
                                            }
                                            // Sort the subjects based on timetable date
                                            asort($subjectsWithDates);

                                            // Extract subject codes in sorted order
                                            $sortedSubjectCodes = implode(', ', array_keys($subjectsWithDates));

                                            // Update $subject_codes with sorted subject codes
                                            $subject_codes = $sortedSubjectCodes;
                                            $subjectnameArray = explode(', ',  $subject_name);
                                            $subjectArray = explode(', ',  $subject_codes);
                                            foreach ($subjectArray as $subject_code) {
                                                $examtt = $con->prepare("SELECT `subject_code` FROM `tbl_exam_timetable` WHERE `subject_code`= ? and `sub_type`= 'Theory' and exam_id = ?");
                                                $examtt->bind_param("si", $subject_code, $exam_id);
                                                $examtt->execute();
                                                $examttres = $examtt->get_result();
                                                while ($rowtt = $examttres->fetch_assoc()) {
                                    ...
                                    <tr>
                                        ...
                                        <?php
                                            ...
                                            $subjectArray = explode(', ',  $subject_codes);
                                            foreach ($subjectArray as $subject_code) {
                                                $examtt = $con->prepare("SELECT `date` FROM `tbl_exam_timetable` WHERE `subject_code`= ? and `sub_type`= 'Theory' and is_active = 1 and exam_id = ?");
                                                $examtt->bind_param("si", $subject_code, $exam_id);
                                                $examtt->execute();
                                                $examttres = $examtt->get_result();
                                                while ($rowtt = $examttres->fetch_assoc()) {
                                    ...
                                    <tr>
                                        ...
                                        <?php
                                            ...
                                            $subjectArray = explode(', ',  $subject_codes);
                                            foreach ($subjectArray as $subject_code) {
                                                $examtt = $con->prepare("SELECT `start_time`, `end_time` FROM `tbl_exam_timetable` WHERE `subject_code`= ? and `sub_type`= 'Theory' and is_active = 1 and exam_id = ?");
                                                $examtt->bind_param("si", $subject_code, $exam_id);
                                                $examtt->execute();
                                                $examttres = $examtt->get_result();
                                                while ($rowtt = $examttres->fetch_assoc()) {
                                ...
                                <?php
                                } elseif ($extype == 'remedial') {
                                ?>
                                    <tr>
                                        <td style="width: 126px;border:1px solid black;">Subject :</td>
                                        <?php
                                        $stmtred = $con->prepare("SELECT subject_code FROM tbl_exam_results WHERE enrollnment_no = ? AND exam_id = ? and is_active = 1");
                                        $stmtred->bind_param("si", $er_no, $exam_id);
                                        $stmtred->execute();
                                    ...
                                    <tr>
                                        ...
                                        foreach ($subject_codes as $subject_code) {
                                            $examtt = $con->prepare("SELECT `date` FROM `tbl_exam_timetable` WHERE `subject_code`= ? and `sub_type`= 'Theory' and exam_id = $exam_id and is_active = 1");
                                    ...
                                    <tr>
                                        ...
                                        <?php
                                        foreach ($subject_codes as $subject_code) {
                                            $examtt = $con->prepare("SELECT `start_time`, `end_time` FROM `tbl_exam_timetable` WHERE `subject_code`= ? and `sub_type`= 'Theory' and exam_id = $exam_id and is_active = 1");
                                            $examtt->bind_param("s", $subject_code);
                                            $examtt->execute();
                                            $examttres = $examtt->get_result();
                                            while ($rowtt = $examttres->fetch_assoc()) {
            ...
            <div class="text-center m-5">
                ...
                <button class="btn btn-primary print-btn" onclick="printA()"><i class="fa fa-print"></i> Print Receipt</button>
            </div>
        <?php
        } ?>
    </div>
    ...
    <script>
        function printA() {
            if (Android !== "undefined" && Android.print) {
                Android.print();
            } else {
                window.print();
            }
