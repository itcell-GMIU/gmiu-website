...
if (isset($_GET['pID'])) {
    ...
    $stmt = $con->prepare("UPDATE `tbl_exam_form` SET hallticket_status = ? WHERE id = ? ");
    $stmt->bind_param("ii", $pStat, $pID);
    $result = $stmt->execute();
...
<body class="hold-transition sidebar-mini layout-fixed">
                        ...
                        <div class="card-body">
                            ...
                            <div class="row">
                                <div class="form-group col-sm-12">
                                    ...
                                    <label for="sem-filter">Filter by Sem:</label>
                                    <select id="sem-filter" class="form-control">
                            ...
                            <table id="acedemic1" class="dataTableLoad table table-bordered table-striped table-responsive">
                                <thead>
                                    <tr align="center">
                                        ...
                                        <th style="color:black;"><b>Practical HT</b></th>
                                        <th style="color:black;"><b>For Student</b></th>
                                    </tr>
                                </thead>
                                ...
                                <tbody>
                                    ...
                                    ?>
                                        <tr align="center">
                                            ...
                                            <td scope="row">
                                                <?php
                                                $sqlx = mysqli_query($con, "SELECT id FROM tbl_exam_results WHERE exam_id = " . $row['id'] . "");
                                                $count = mysqli_num_rows($sqlx);

                                                if ($count > 0) {
                                                    echo '<span class="badge badge-success">Published</span>
                                                    <hr>
                                                    <a href="update_hall_ticket.php?id=' . $row['id'] . '" class="btn btn-info text-nowrap"><i class="fas fa-refresh"></i> Publish</a>
                                                ';
                                                } else { ?>
                                                    <a href="update_hall_ticket.php?id=<?php echo $row['id'] ?>" class="btn btn-success text-nowrap"><i class="fas fa-bullhorn"></i> Publish</a>
                                                <?php } ?>
                                            </td>
                                            <td scope="row">
                                                <?php if ($PRhallticket_status == 1) {
                                                    echo '<span class="badge badge-success">Published</span>';
                                                } elseif ($PRhallticket_status == 0) { ?>
                                                    <a href="update_prhall_ticket.php?id=<?php echo $row['id'] ?>" class="btn btn-success text-nowrap"><i class="fas fa-bullhorn"></i> Publish</a>
                                                <?php } ?>
                                            </td>
                                            <td>
                                                <?php if ($hallticket_status == 0) {
                                                    echo '
                                                    <a href="publish_hall_ticket.php?pID=' . $row['id'] . '&ps=1" class="btn btn-success">Active</a>
                                                ';
                                                } elseif ($hallticket_status == 1) { ?>
                                                    <span class="badge badge-success">Activated</span>
                                                    <hr>
                                                    <a href="publish_hall_ticket.php?pID=<?= $row['id'] ?>&ps=0" class="btn btn-success">De-Active</a>
                                                <?php } ?>
                                            </td>
                                        </tr>
                                    <?php } ?>
    ...
    <script>
        $(document).ready(function() {
            ...
            // Add dropdown filter for the "Game" column
            table.column(2).data().unique().sort().each(function(value, index) {
                $('#sem-filter').append('<option value="' + value + '">' + value + '</option>');
            });

            // Apply the filter when the dropdown value changes
            $('#sem-filter').on('change', function() {
