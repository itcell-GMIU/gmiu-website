    ...
    while ($stdr = $resultstd->fetch_assoc()) {
        ...
        while ($row2 = $result2->fetch_assoc()) {
            ...
            foreach ($subjectArray as $subject) {
                ...
                if ($recordCount == 0) {
                    $insertQuery = "INSERT INTO `tbl_exam_results`(`enrollnment_no`, `seat_no`, `barcode`, `subject_code`, `exam_id`) VALUES ('$enrollment','$seat_no','$barcode','$subject','$id')";
                    $con->query($insertQuery);
                } else {
                    $updateQuery = "UPDATE `tbl_exam_results` SET `barcode`='$barcode', `subject_code`='$subject' WHERE `enrollnment_no`='$enrollment' AND `seat_no`='$seat_no' AND `exam_id`='$id'";
                    $con->query($updateQuery);
                }
