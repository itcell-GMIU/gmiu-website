...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['inquiry_id'], $_POST['staff_id'])) {
    ...
    if (empty($row['staff_id'])) {
        echo json_encode([
            'status' => 'notassigned',
            'message' => 'This inquiry is Not Assigned to any Staff !!!.'
        ]);
        exit;
    } else {
    ...
    try {
        ...
        $log_stmt = $con->prepare("INSERT INTO tbl_inquiry_assign_log 
                                        (assign_by, old_staff_id, staff_id, starting_id, total, created_at)
                                        VALUES (?, ?, ?, ?, ?, NOW())
        ");
        $total = 1;
        $log_stmt->bind_param("iiisi", $assign_by, $old_staff_id, $staff_id, $inquiry_id, $total);
