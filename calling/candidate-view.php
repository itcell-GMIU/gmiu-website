...
if (($role_id == 15 || $role_id == 16) && ($url_for == '' || $url_for == 'inq' || $url_for == 'notassignedinq')) {
    ...
    echo "<a href='candidate-view.php?url_for=rejectedinq'>rejected inquiry</a><br>";
    if ($role_id != 14) {
        echo "<a href='candidate-view.php?url_for=notassignedinq'>not assigned inquiry</a><br>";
    }
    echo "<a href='candidate-view.php?url_for=closedinq'>closed inquiry</a><br>";
    exit;
}
                ...
                <div class="pd-20 bg-white border-radius-4 box-shadow mb-30">
                    ...
                    <div class="row mb-3 align-items-end">
                        <?php if ($role_id == 15 || $role_id == 16) { ?>
                            <div class="col-md-3">
                                <label for="assignedDateFilter"><strong>Filter by Assigned Date:</strong></label>
                                <input type="date" id="assignedDateFilter" class="form-control">
                            </div>
                            <div class="col-md-3">
                                <label for="callCountFilter"><strong>Call Count:</strong></label>
                                <select id="callCountFilter" class="form-control">
                                    <option value="">Select Call Count</option>
                                    <option value="1">1 Call</option>
                                    <option value="2">2 Calls</option>
                                    <option value="3">3 Calls</option>
                                    <option value="3+">More than 3 Calls</option>
                                </select>
                            </div>
                        <?php } ?>
                        <div class="col-md-3">
                            <label for="inquiryType"><strong>Type Of Inquiry:</strong></label>
                        ...
                        <div class="col-md-2">
                            ...
                            <button id="clearDateFilter" class="btn btn-secondary ml-1">Clear</button>
                        </div>
                    </div>
                ...
                <div class="modal-content">
                    ...
                    <form id="callStatusForm" method="POST">
                        ...
                        <div class="modal-footer">
                            ...
                            <button id="callFormSubmit" type="button" class="btn btn-primary" name="callForm">Save</button>
            ...
            $('#candidateTable').DataTable({
                ...
                ajax: {
                    ...
                    data: function (d) {
                        ...
                        d.assigned_date = $('#assignedDateFilter').val() || '';
                        d.call_count = $('#callCountFilter').val() || '';
                        d.inquiry_type = $('#inquiryType').val() || '';
                    },
                    dataSrc: function (json) {
                        // console.log("DataTables Fetch Result:", json);
                        return json.data || [];
                    },
                    error: function (xhr, error, thrown) {
                        swal({
                            title: 'Data Load Error',
                            text: 'Something went wrong while fetching the candidate data. Please check your connection or contact ITCELL.' + error,
                            type: 'error',
    ...
    if ($role_id == 15 || $role_id == 16) {
        ...
        <script>
            $(document).ready(function () {
                ...
                $('#callFormSubmit').on('click', function (e) {
                    e.preventDefault();

                    let formData = {
                        callForm: true,
                        tis_id: $('#tis_id_modal_openCallStatusModal').val(),
                        inq_student_id: $('#inq_student_id_hidden_openCallStatusModal').val(),
                        call_status: $('#call_status_input').val(),
                        remark: $('#call_remark_input').val(),
                        conversation: $('#conversation_input').val(),
                        other_remark: $('#other_remark').val()
                    };

                    $.ajax({
                        url: './extra/add-call-logs.php',
                        type: 'POST',
                        data: formData,
                        dataType: 'json',

                        beforeSend: function () {
                            $('#callFormSubmit').prop('disabled', true);
                        ...
                        success: function (response) {
                            ...
                            if (response.status === 'success') {
                                ...
                                // 🔥 HERE IS YOUR ANSWER (frontend update)
                                updateCallLogUI(formData);
                        ...
                        complete: function () {
                            $('#callFormSubmit').prop('disabled', false);
                        }
                    });
                });
            });

            function updateCallLogUI(formData) {
                ...
                table.rows().every(function () {
                    ...
                    if (row.inq_student_id == formData.inq_student_id) {

                        // ✅ increase call count
                        row.call_count = parseInt(row.call_count || 0) + 1;

                        // ✅ if CLOSED
                        if (formData.call_status === 'CLOSE') {
                            row.is_closed = 1;
                            row.admission_status = 'Closed';
                        }
