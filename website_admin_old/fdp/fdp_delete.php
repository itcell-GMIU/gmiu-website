...
if (isset($_GET['fdp_id']) && !empty($_GET['fdp_id'])) {
    $id = mysqli_real_escape_string($con, $_GET['fdp_id']);
    $id = only_digits($id);
    if ($id == false) {
...
if ($result) {
    ...
    $cmd = $con->prepare("SELECT photos.file_name FROM `tbl_site_photos` AS photos INNER JOIN `tbl_campus` AS fdp 
    ON photos.type_id = fdp.id WHERE photos.type = ? AND photos.type_id = ?");
    ...
    while ($row = $result2->fetch_assoc()) {
      ...
      if (delete_file($file_name, $path)) {
        ...
        if ($file_delete) {
          $_SESSION['status'] = "fdp Activity is deleted successfully";
          $_SESSION['status_code'] = "success";
          
          // Delete the report file if it exists
          if (!empty($report_name)) {
            $report_path = '../uploads/fdp/';
            if (delete_file($report_name, $report_path)) {
             
            }
          }
        } else {
          $_SESSION['status'] = "fdp Activity deletion failed";
          $_SESSION['status_code'] = "error";
        }
      } else {
        $_SESSION['status'] = "fdp Activity deletion failed";
    ...
    // Redirect to fdp Activity view page after processing
    echo "<script>setTimeout(function(){window.location='fdp_view.php'}, 1000);</script>";
