...
if (isset($_POST['submit'])) {
    ...
    $stmt = $con->prepare("UPDATE `tbl_exam_paper` SET faculty_id = ?, level_id = ?, program_id = ?, document = ?, title = ? WHERE id = ? "); // Prepare the update statement
    $stmt->bind_param("iiissi", $faculty_name, $level_name, $program_name, $paper_title, $document, $paper_id); // Bind the parameters for the prepared statement
