...
if ($role_id == 8) {
    if (isset($_POST['submit'])) {
        ...
        $stmt = $con->prepare("INSERT INTO `tbl_exam_paper`(faculty_id, level_id, program_id, year, session, title, document) VALUES (?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("iiissss", $faculty_id, $level_id, $program_id, $year, $session, $title, $document);
        $result = $stmt->execute();
        if ($result) {
