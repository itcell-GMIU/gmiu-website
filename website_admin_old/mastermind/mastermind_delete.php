...
if ($result) {
    ...
    while ($row = $result2->fetch_assoc()) {
      ...
      if (delete_file($file_name, $path)) {
        ...
        if ($file_delete) {
          $_SESSION['status'] = "mastermind Activity is deleted successfully";
          $_SESSION['status_code'] = "success";
          
          // Delete the report file if it exists
          if (!empty($report_name)) {
            $report_path = '../uploads/mastermind/';
            if (delete_file($report_name, $report_path)) {
             
            }
          }
        } else {
          $_SESSION['status'] = "mastermind Activity deletion failed";
          $_SESSION['status_code'] = "error";
        }
      } else {
        $_SESSION['status'] = "mastermind Activity deletion failed";
    ...
    // Redirect to mastermind Activity view page after processing
    echo "<script>setTimeout(function(){window.location='mastermind_view.php'}, 1000);</script>";
