...
if (isset($_POST["submit"])) {
    ...
    if ($type_file == 'video') {
        ...
        $stmt = $con->prepare("INSERT INTO `tbl_daily_post`(date,file_type,file,year,month,faculty_id, level_id, program_id, is_common_reel)VALUES (?,?,?,?,?,?,?,?,?)");
        $stmt->bind_param("sssiiissi", $date, $type_file, $video_link, $year, $month, $faculty, $level, $program, $is_common);
        $result = $stmt->execute();
        ...
        $stmt = $con->prepare("INSERT INTO `tbl_daily_post`(date,file_type,year,month,faculty_id, level_id, program_id, is_common_reel)VALUES (?,?,?,?,?,?,?,?)");
        $stmt->bind_param("ssiiissi", $date, $type_file, $year, $month, $faculty, $level, $program, $is_common);
        $result = $stmt->execute();
