...
if ($result) {
    ...
    $cmd = $con->prepare("SELECT photos.file_name FROM `tbl_site_photos` AS photos INNER JOIN `tbl_campus` AS sports 
    ON photos.type_id = sports.id WHERE photos.type = ? AND photos.type_id = ?");
    ...
    //   $report_name = $row['report'];
      ...
      if (delete_file($file_name, $path)) {
        ...
        if ($file_delete) {
          $_SESSION['status'] = "Sports Activity is deleted successfully";
          $_SESSION['status_code'] = "success";
          
          // Delete the report file if it exists
          if (!empty($report_name)) {
            $report_path = '../uploads/sports/';
            if (delete_file($report_name, $report_path)) {
             
            }
          }
        } else {
          $_SESSION['status'] = "Sports Activity deletion failed";
          $_SESSION['status_code'] = "error";
        }
      } else {
        $_SESSION['status'] = "Sports Activity deletion failed";
    ...
    // Redirect to Sports Activity view page after processing
    echo "<script>setTimeout(function(){window.location='sports_view.php'}, 1000);</script>";
