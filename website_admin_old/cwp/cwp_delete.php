...
if (isset($_GET['cwp_id']) && !empty($_GET['cwp_id'])) {
    $id = mysqli_real_escape_string($con, $_GET['cwp_id']);
    $id = only_digits($id);
    if ($id == false) {
...
if ($result) {
    ...
    $cmd = $con->prepare("SELECT photos.file_name FROM `tbl_site_photos` AS photos INNER JOIN `tbl_campus` AS cwp 
    ON photos.type_id = cwp.id WHERE photos.type = ? AND photos.type_id = ?");
    ...
    while ($row = $result2->fetch_assoc()) {
      ...
      if (delete_file($file_name, $path)) {
        ...
        if ($file_delete) {
          $_SESSION['status'] = "cwp Activity is deleted successfully";
          $_SESSION['status_code'] = "success";
          
          // Delete the report file if it exists
          if (!empty($report_name)) {
            $report_path = '../uploads/cwp/';
            if (delete_file($report_name, $report_path)) {
             
            }
          }
        } else {
          $_SESSION['status'] = "cwp Activity deletion failed";
          $_SESSION['status_code'] = "error";
        }
      } else {
        $_SESSION['status'] = "cwp Activity deletion failed";
    ...
    // Redirect to cwp Activity view page after processing
    echo "<script>setTimeout(function(){window.location='cwp_view.php'}, 1000);</script>";
