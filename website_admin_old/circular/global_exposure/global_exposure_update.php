...
if (isset($_POST['submit'])) {
    ...
    if (!empty($image_upload['name'])) {
        ...
        if ($file_upload_status['status'] == 200) {
            ...
            echo "<script>setTimeout(function(){window.location='global_exposure_edit.php?ic_id=$id'},1000)</script>";
            exit();
        }
    } else {
    ...
    if ($result) {
        $_SESSION['status'] = "Global Exposure Updated Successfully for ";
        $_SESSION['status_code'] = "success";
        echo "<script>setTimeout(function(){window.location='global_exposure_view.php'},1000);</script>";
    } else {
        // Check for SQL errors
        $_SESSION['status'] = "Global Exposure Updation Failed: " . $stmt->error;
        $_SESSION['status_code'] = "error";
        echo "<script>setTimeout(function(){window.location='global_exposure_edit.php?ic_id=$id'},1000)</script>";
    }
}
