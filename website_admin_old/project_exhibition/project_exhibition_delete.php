...
if ($result) {
    ...
    while ($row = $result2->fetch_assoc()) {
      ...
      $path = '../uploads/project_exhibition/';
      ...
      if (delete_file($file_name, $path)) {
        ...
        if ($file_delete) {
          $_SESSION['status'] = "project_exhibition Activity is deleted successfully";
          $_SESSION['status_code'] = "success";
          
          // Delete the report file if it exists
          if (!empty($report_name)) {
            $report_path = '../uploads/project_exhibition/';
            if (delete_file($report_name, $report_path)) {
             
            }
          }
        } else {
          $_SESSION['status'] = "project_exhibition Activity deletion failed";
          $_SESSION['status_code'] = "error";
        }
      } else {
        $_SESSION['status'] = "project_exhibition Activity deletion failed";
    ...
    // Redirect to project_exhibition Activity view page after processing
    echo "<script>setTimeout(function(){window.location='project_exhibition_view.php'}, 1000);</script>";
