...
if (isset($_GET['ic_id']) && !empty($_GET['ic_id'])) {
    ...
    if ($id == false) {
        ...
        echo "<script>seicimeout(function(){window.location='international_cell_view.php'},1000)</script>";
    }
...
if ($result) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if (delete_file($img_name, $path)) {
            $_SESSION['status'] = "international Relation cell Delete Successfully";
    $_SESSION['status_code'] = "success";
    echo "<script>setTimeout(function(){window.location='international_cell_view.php'},1000);</script>";
        } else {
            $_SESSION['status'] = "international Relation cell Deletion Failed";
    $_SESSION['status_code'] = "error";
