...
if (isset($_GET['mock_id']) && !empty($_GET['mock_id'])) {
  ...
  $stmt = $con->prepare("UPDATE `tbl_interview_alumni` SET is_delete = 1, is_active = 0 WHERE id = ?");
  ...
  if ($result) {
    ...
    $cmd = $con->prepare("SELECT photos.file_name, mock.report_file as report FROM `tbl_site_photos` AS photos INNER JOIN `tbl_interview_alumni` AS mock ON photos.type_id = mock.id WHERE photos.type = ? AND photos.type_id = ?");
    $cmd->bind_param("si", $type, $mock_id);
    $cmd->execute();
    ...
    while ($row = $result2->fetch_assoc()) {
      ...
      $path = '../uploads/interview_alumni/image/';
      ...
      if (delete_file($file_name, $path)) {
        ...
        $stmt->bind_param("is", $mock_id, $type);
        ...
        if ($file_delete) {
          ...
          if (!empty($report_name)) {
            $report_path = '../uploads/interview_alumni/report/';
