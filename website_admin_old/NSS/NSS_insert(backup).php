...
if (isset($_POST['submit'])) {
    ...
    $stmt = $con->prepare("INSERT INTO `tbl_NSS`(report_title,date)VALUES (?,?)");
    $stmt->bind_param("ss",$title, $date);
    $result = $stmt->execute();
    $id = $con->insert_id;
    if ($_FILES['report_upload']['error'] == 0 && $_FILES['image_upload']['error'] == 0) {
        ...
        if ($file_upload_status['status'] == 200) {
            $report_name = $file_upload_status['message'];
            $stmt = $con->prepare("UPDATE `tbl_NSS` SET `report_thumbnail` = ? WHERE `tbl_NSS`.`id` = ?");
            $stmt->bind_param("si", $report_name, $id);
            $result = $stmt->execute();
            if ($result) {
                                ...
                                <form id="quickForm" method="POST" enctype="multipart/form-data">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div name="image1" id="image1" class="form-group">
                                            <div class="form-group">
                                                <label for="exampleInputFile">Upload Report Thumbnail <span style="color: red;">*</span></label>
                                                <div class="input-group">
                                                    <div class="custom-file">
                                                        <input type="file" class="custom-file-input" name="image_upload" id="image_upload" require>
