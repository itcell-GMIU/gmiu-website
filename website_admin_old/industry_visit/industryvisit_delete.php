...
if (isset($_GET['visit_id']) && !empty($_GET['visit_id'])) {
  $id = mysqli_real_escape_string($con, $_GET['visit_id']);
  $id = only_digits($id);
  if ($id == false) {
...
$stmt = $con->prepare("UPDATE `tbl_industry_visit` SET is_delete = 1 , is_active = 0 WHERE id = ? ");
...
if ($result) {
  ...
  $cmd = $con->prepare("SELECT report  FROM `tbl_industry_visit` WHERE id = ?");
  $cmd->bind_param("i", $id);
  $cmd->execute();
  ...
  if ($report) {
    while ($row = $report->fetch_assoc()) {
      if (!empty($row['report'])) {
        ...
        $path = '../uploads/industry_visit/report';
  ...
  while ($row = $result2->fetch_assoc()) {
    ...
    $path = '../uploads/industry_visit/image/';
    ...
    if (delete_file($file_name, $path)) {
      ...
      if ($file_delete) {
        $_SESSION['status'] = " Industry Visit Delete Successfully";
        $_SESSION['status_code'] = "success";
       echo "<script>setTimeout(function(){window.location='industryvisit_view.php'},1000);</script>";
      } else {
        $_SESSION['status'] = " Industry Visit Deletion Failed";
        $_SESSION['status_code'] = "error";
        echo "<script>setTimeout(function(){window.location='industryvisit_view.php'},1000);</script>";
      }
    } else {
