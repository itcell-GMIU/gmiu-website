...
if (isset($_POST['submit'])) {
    ...
    $check = $con->prepare("SELECT * FROM `tbl_inquiry_student` where inq_student_id BETWEEN ? AND ? ");
    $check->bind_param("ss", $startingInquiryId, $endingInquiryId);
    $check->execute();
    ...
    if (is_null($exist_id)) {
        // Perform your query
        $stmt = $con->prepare("UPDATE tbl_inquiry_student SET staff_id = ?, assign_by = ? , ip_address_column= ? WHERE inq_student_id BETWEEN ? AND ?");
