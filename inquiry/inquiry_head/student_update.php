...
if (isset($_POST['submit_std'])) {
    ...
    // } else {
        ...
        $stmt->bind_param("isssssssssiisi", $last_exam, $first_name, $middle_name, $last_name, $gender, $mobile_number, $second_mobile_number, $email, $faculty_id, $level_id, $program_id, $last_exam_marks, $is_online, $inq_id);
        $result = $stmt->execute();
        if ($result) {
            if ($result) {
                ...
                echo "<script>setTimeout(function(){window.location='assign_student.php'},1000);</script>";
            } else {
