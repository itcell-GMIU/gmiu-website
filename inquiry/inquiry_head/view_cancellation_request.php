...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'update_hod_status') {
    ...
    $stmt = $con->prepare("UPDATE tbl_cancellation_requests SET hod_status = ?, hod_remark = ?, hod_at = NOW(), updated_at = NOW() WHERE id = ?");
...
ON
    FIND_IN_SET(p.id, ts.program_id)
WHERE
    ts.role_id = 14 AND tcr.is_active = 1 AND tcr.is_delete = 0 AND ts.id = ?
");
                                            ...
                                            <tbody>
                                                ...
                                                if ($result->num_rows > 0) {
                                                    while ($row = $result->fetch_assoc()) {
                                                        ...
                                                        $clusterStatus = $row['cluster_status']; // Get the cluster admin approval status
                                                
                                                        if ($status === 'pending') {
                                                            echo "<button class='btn btn-success btn-sm change-hod-status' data-id='" . $row['tcr_id'] . "' data-status='Approved' title='Approve'>
                                                              <i class='fa fa-check'></i></button>";
                                                            echo "<button class='btn btn-danger btn-sm change-hod-status' data-id='" . $row['tcr_id'] . "' data-status='Rejected' title='Reject'>
                                                              <i class='fa fa-times'></i></button>";
                                                        } elseif ($clusterStatus == 'pending') {
                                                            if ($status == 'rejected') {
                                                                echo "<button class='btn btn-success btn-sm change-hod-status' data-id='" . $row['tcr_id'] . "' data-status='Approved' title='Approve'>
                                                               <i class='fa fa-check'></i> </button>";
