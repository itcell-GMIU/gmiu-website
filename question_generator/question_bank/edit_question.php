...
std.marks as marks,
std.co_level as co_level,
std.bl_level as bl_level
...
while ($row = $result->fetch_assoc()) {
    ...
    $co_level = !empty($row['co_level']) ? $row['co_level'] : 'N/A';
    $bl_level = !empty($row['bl_level']) ? $row['bl_level'] : 'N/A';
}
...
<head>
    ...
    <!--            removePlugins: 'image,easyimage,cloudservices',-->
                <!--filebrowserUploadUrl: 'ckupload.php', // ✅ for file browser uploads-->
    <!--            filebrowserUploadMethod: 'form',-->
                <!--uploadUrl: 'ckupload.php', // ✅ for drag/drop upload-->
                <!--imageUploadUrl: 'ckupload.php', // ✅ required for image2 plugin-->
    <!--            fileTools_requestHeaders: {-->
    <!--                'Cache-Control': 'no-cache'-->
    <!--            }-->
    <!--        });-->
    <!--        CKEDITOR.instances.text_editor.on('fileUploadResponse', function(evt) {-->
                <!--evt.stop(); // prevent default behavior-->
    <!--            var data = evt.data,-->
    <!--                xhr = data.fileLoader.xhr,-->
    <!--                response = JSON.parse(xhr.responseText);-->

    <!--            if (response.uploaded && response.url) {-->
                    <!--data.url = response.url; // ✅ Force CKEditor to insert the URL-->
    <!--            } else {-->
    <!--                alert("Upload failed: " + (response.error?.message || "Unknown error"));-->
    <!--            }-->
    <!--        });-->
    <!--    });-->
    <!--</script>-->
                                    ...
                                    <div class="card-body">
                                        <div class="form-group row">
                                            ...
                                            <div class="form-group col-md-4">
                                                ...
                                                <input type="text" name="marks" value="<?php echo $marks; ?> " class="form-control" id="marks" placeholder="Enter CO Level" required>
                                            </div>
                                            <div class="form-group col-md-4">
                                                <label for="co_level">Course Outcomes Level<span style="color: red;">*</span></label>
                                                <input type="text" name="co_level" value="<?php echo $co_level; ?> " class="form-control" id="co_level" placeholder="Enter CO Level" required>
                                            </div>
                                            <div class="form-group col-md-4">
