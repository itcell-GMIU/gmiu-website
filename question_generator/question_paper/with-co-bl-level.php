...
if ($role_id == 51) {
    ...
    // }
     ...
     $question_stmt = $con->prepare("
    SELECT pq.question_id, pq.sequence_no, pq.bl_level,
           q.id, q.question, q.modify_question, q.marks, q.chapter AS co
            FROM tbl_paper_question pq
            LEFT JOIN tbl_questions q ON pq.question_id = q.id
            WHERE pq.paper_id = ?
            ORDER BY pq.sequence_no ASC
            ");
    $question_stmt->bind_param("i", $Id);
    $question_stmt->execute();
    $questions_result = $question_stmt->get_result();
    ...
    if ($allZero) {
        // DEFAULT ORDER (original fetch order)
        foreach ($questions as $q) {
            if ($q['marks'] == 5)
                $questionsByMarks[5][] = $q;
            else if ($q['marks'] == 10)
        ...
        // SEQUENCE ORDER (already ordered by sequence_no in SQL)
        foreach ($questions as $q) {
            if ($q['sequence_no'] > 0) {
                if ($q['marks'] == 5)
                    $questionsByMarks[5][] = $q;
                ...
                <thead>
                    ...
                    <tr class="header-info">
                        ...
                        <th style="padding: 0px 8px 8px 8px; text-align: left; vertical-align: top;">
                            <?php } ?>
                           Date: <?php echo date('d/m/Y', strtotime($exam_date)); ?><br>
                    ...
                    foreach ($format as $qFormat) {
                        ...
                        foreach ($parts as $item) {
                            ...
                            if (!empty($questionsByMarks[$mark])) {
                                ...
                                $sub = !empty($qData['modify_question']) ? $qData['modify_question'] : $qData['question'];
