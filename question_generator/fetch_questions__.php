    ...
    questionCheckboxes.forEach(function(checkbox) {
        ...
        function updateFooterRows() {
            ...
            if (totalMarks == 175 || totalMarks == 100) {
                ...
                // Prepare a string to display all chapters in one <td>
                let chapterContent = "<td>Chapter Weightage:</td>";
                let chapterDetails = "";

                for (var chapter in chapterMarks) {
                    if (chapterMarks.hasOwnProperty(chapter)) {
                        chapterDetails += ` ${chapter} : ${chapterMarks[chapter]} ||  `;
                    }
                }

                // Remove the last comma and space
                chapterDetails = chapterDetails.slice(0, -2);

                // Add all chapters as one <td>
                chapterRow.innerHTML = `${chapterContent}<td colspan = "6">${chapterDetails}</td>`;
            } else {
