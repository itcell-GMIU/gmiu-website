...
if (isset($_POST['subject_code']) && isset($_POST['t_marks'])) {
    ...
    if ($totalMarks == 90 || $totalMarks == 180) {
        ...
        // Add remaining questions to the first chapter
        $remainingQuestions = $maxLevel - $totalQuestions;

        // Identify the first chapter (first key in $totalQuestionsPerChapter)
        $firstChapter = array_key_first($totalQuestionsPerChapter);

        // Add the remaining questions to the first chapter
        ...
        // ✅ Final adjustment to ensure total marks exactly match $totalMarks
        // $currentTotal = array_sum($chapterMarks);
        // $difference = $totalMarks - $currentTotal;
        
        // echo "<script>";
        // echo "console.log('Initial Chapter Marks: ', " . json_encode($chapterMarks) . ");";
        // echo "console.log('Initial Total: $currentTotal');";
        // echo "console.log('Expected Total: $totalMarks');";
        // echo "console.log('Initial Difference: $difference');";
        // echo "</script>";
        
        // if ($difference !== 0) {
        //     $chapterKeys = array_keys($chapterMarks);
        //     $index = 0;
        //     $loopCount = 0; // Safety to avoid infinite loop

        //     while ($difference !== 0 ) {
        //         $chapter = $chapterKeys[$index % count($chapterKeys)];
                
        //          // Debug log for each step
        // echo "<script>console.log('Loop $loopCount - Chapter: $chapter, Current Marks: {$chapterMarks[$chapter]}, Difference: $difference');</script>";

        //         // Increase or decrease only if it keeps multiples of 5 and valid marks
        //         if ($difference > 0) {
        //             $chapterMarks[$chapter] += 5;
        //             $difference -= 5;
        //         } elseif ($difference < 0 && $chapterMarks[$chapter] >= 10) {
        //             $chapterMarks[$chapter] -= 5;
        //             $difference += 5;
        //         }
        ...
        //     echo "<script>";
        //     echo "console.log('Final Adjusted Chapter Marks: ', " . json_encode($chapterMarks) . ");";
        //     echo "console.log('Final Total: " . array_sum($chapterMarks) . "');";
        //     echo "console.log('Remaining Difference: $difference');";
        //     echo "</script>";
        // }
    }
    // Pass the JSON strings to JavaScript as variables
    ...
    echo '<style>
        ...
        tfoot {
            position: sticky;
            bottom: 0;
            background-color: #f1f1f1; /* Optional: to give it some background */
            padding: 10px; /* Optional: to give it some padding */
            box-shadow: 0 -2px 5px rgba(0,0,0,0.1); /* Optional: to give it a slight shadow */
            width: 100%;
        }
...
<script>
    ...
    questionCheckboxes.forEach(function(checkbox) {
            ...
            if (totalMarks == 175 || totalMarks == 100) {
                ...
                if (checkbox.checked) {
                    ...
                    if (blMarks[blLevel] < 0) {
                        ...
                        swal("Deducting this mark will result in negative Bloom's Level . Please select another question.");
                    ...
                    if (chapterMarks[chapter] < 0) {
                        ...
                        swal("Deducting this mark will result in negative chapter weightage. Please select another question.");
                        // Add the mark back to Bloom's Level and chapter weightage
                        blMarks[blLevel] += parseInt(row.cells[3].textContent.trim());
        ...
        function updateFooterRows() {
            ...
            // Display total marks
            var totalMarksRow = footer.querySelector(".total-marks-row");
            if (!totalMarksRow) {
                totalMarksRow = document.createElement("tr");
                totalMarksRow.className = "total-marks-row";
                footer.appendChild(totalMarksRow);
            } else {
                totalMarksRow.innerHTML = ''; // Clear existing content if any
            }
