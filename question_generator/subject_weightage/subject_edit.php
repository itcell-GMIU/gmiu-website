...
$cmd = $con->prepare("SELECT std.id as id, std.faculty_id as faculty_id, std.level_id as level_id, std.program_id as program_id, std.sem as sem, std.subject_code as subject_code, std.subject_name as subject_name FROM tbl_std_corner_exam as std WHERE std.id = ?");
$cmd->bind_param("i", $id);
$cmd->execute();
