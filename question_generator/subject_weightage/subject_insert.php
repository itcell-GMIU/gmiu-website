...
if (isset($_POST['submit'])) {
    ...
    $stmt = $con->prepare("SELECT * FROM `tbl_std_corner_exam` WHERE subject_code = ? And 
    faculty_id = ?  and level_id = ? and program_id = ? AND is_delete = '0'  ");
    $stmt->bind_param("siii", $subject_code, $faculty_id, $level_id, $program_id);
    $stmt->execute();
    ...
    if ($result->num_rows > 0) {
        ...
        $stmt = $con->prepare("INSERT INTO `tbl_std_corner_exam` (faculty_id, level_id, program_id, sem, subject_code, subject_name) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("iiiiss", $faculty_id, $level_id, $program_id, $sem, $subject_code, $subject_name);
        $result1 = $stmt->execute();

        if ($result1) {
            $_SESSION['status'] = "Subject Inserted Successfully";
            $_SESSION['status_code'] = "success";
            echo "<script>setTimeout(function(){window.location='subject_view.php'},1000);</script>";
        } else {
            $_SESSION['status'] = "Subject Insertion Failed";
            $_SESSION['status_code'] = "error";
            echo "<script>setTimeout(function(){window.location='subject_view.php'},1000);</script>";
        }
    }
