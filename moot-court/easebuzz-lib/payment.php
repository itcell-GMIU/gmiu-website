...
function initiate_payment($params, $redirect, $merchant_key, $salt, $env)
{
    ...
    if ($redirect) {
        ...
        // Check if result is an array (error) or object (success)
        if (is_array($result)) {
...
function _pay($params_array, $redirect, $salt_key, $url)
{
    ...
    // Check if curl_result is an array (error) or object (success)
    if (is_array($curl_result)) {
        return $curl_result; // Return error array directly
    }
...
function _paymentResponse($result)
{
    ...
    // Check if result is an array (error from cURL) or object (success)
    if (is_array($result)) {
